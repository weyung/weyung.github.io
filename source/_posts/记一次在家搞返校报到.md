...
第二天睡醒继续搞，思路是要拿到**前端的源代码**，才能搞懂他的逻辑。根据群友 @xy3 提供的文章 <https://blog.cccyun.cn/m/?post=473> 试了一下小程序解包，发现 [wedecode](https://github.com/biggerstar/wedecode) 已经可以直接一把梭了，不用文章前面提到的解密程序处理。
...
1. 往 `/sign/submitPosition` 发一个带学号、活动 ID、WIFI 名（后台决定是否启用）和经纬度信息的请求；
...
一开始没看出来不需要发送位置信息，一直在研究 `/sign/submitPosition` 路由的请求，但是把经纬度改了几次都不对，百度地图和谷歌地图的经纬度都试过了，然后忽然想到他用的是**腾讯位置服务**，于是在网站 <https://lbs.qq.com/getPoint/> 用坐标拾取器拿到经纬度交上去就通过了。
