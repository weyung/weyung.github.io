...
由常见傅里叶变换对
$$
e^{-\alpha t} u(t) \longleftrightarrow{\mathcal{F}} \frac{1}{\alpha + \mathrm{j}\omega}
$$
可知
$$
e^{-2t} u(t) \longleftrightarrow{\mathcal{F}} \frac{1}{2 + \mathrm{j}\omega}
$$
又由**时移性质** $x(t - t_0) \longleftrightarrow{\mathcal{F}} e^{-\mathrm{j}\omega t_0} X(\mathrm{j}\omega)$，可得
$$
...
那么有
$$
e^{-2(t-1)} u(t-1) \longleftrightarrow{\mathcal{F}} \frac{e^{-\mathrm{j}\omega}}{2 - \mathrm{j}\omega} + \frac{e^{-\mathrm{j}\omega}}{2 + \mathrm{j}\omega}
$$
...
$$
\delta(t+1) \longleftrightarrow{\mathcal{F}} e^{\mathrm{j}\omega} \quad \delta(t-1) \longleftrightarrow{\mathcal{F}} e^{-\mathrm{j}\omega}
$$
又由欧拉公式 $e^{\mathrm{j}\theta} = \cos\theta + \mathrm{j}\sin\theta$，可得
$$
\delta(t+1) + \delta(t-1) \longleftrightarrow{\mathcal{F}} 2\cos\omega
$$
...
注意这里提到的性质就是 $y(t) = x(t) * h(t) \longleftrightarrow{\mathcal{F}} Y(\mathrm{j}\omega) = X(\mathrm{j}\omega)H(\mathrm{j}\omega)$。

(1) $x(t) = te^{-2t}u(t), \quad h(t) = e^{-4t}u(t)$
由常见傅里叶变换对
$$
t e^{-\alpha t} u(t) \longleftrightarrow{\mathcal{F}} \frac{1}{(\alpha + \mathrm{j}\omega)^2}
$$
可得
$$
...
注意到积分部分就是 $x(t)*z(t)$，又由卷积性质 $x(t)*z(t) \longleftrightarrow{\mathcal{F}} X(\mathrm{j}\omega)Z(\mathrm{j}\omega)$，所以
$$
...
**习题** 5.18
已知
$$ a^n \longleftrightarrow{\mathcal{F}} \frac{1-a^2}{1-2a \cos \omega + a^2}, \quad |a| < 1 $$
