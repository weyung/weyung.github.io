...
x(t) &= \sum_{k=-\infty}^{\infty} a_k e^{\mathrm{j}k\omega_0 t} = \sum_{k=-\infty}^{\infty} a_k e^{\mathrm{j}k(2\pi/8)t} \\\\
     ...
     &= \mathrm{j} e^{\mathrm{j}(\pi/4)t} - \mathrm{j} e^{-\mathrm{j}(\pi/4)t} + 2 e^{\mathrm{j}(\pi/4)t} + 2 e^{\mathrm{j}5(\pi/4)t} + 2 e^{-\mathrm{j}5(\pi/4)t} \\\\
...
$$
x(t) = \left( \frac{1}{2} \right)^n u[n] = \sum_{k=-\infty}^{\infty} \delta(t - n)
$$
其中 $a_1 = \frac{1}{2}$，其余 $a_k = 0$。

(b) 求 $y(t)$ 的傅里叶级数系数。
利用欧拉公式对 $ \sin $ 函数展开，有：
$$
y(t) = \frac{1}{2j} \left( e^{j4\pi t} - e^{-j4\pi t} \right) = \sum_{k=-\infty}^{\infty} b_k e^{jk\omega_0 t}
$$
其中 $b_{-1} = \frac{1}{2j}$，$b_1 = -\frac{1}{2j}$，其余 $b_k = 0$。

(c) 利用 (a) 和 (b) 的结果，按照连续时间傅里叶级数的相乘性质，求 $z(t) = x(t) y(t)$ 的傅里叶级数系数。
$$
z(t) = x(t)y(t) \stackrel{\text{FS}}{=} \sum_{k=-\infty}^{\infty} c_k e^{jk\omega_0 t}
$$
其中
$$
c_k = \sum_{n=-\infty}^{\infty} a_n b_{k-n} = \left( \frac{1}{2} \right) \left( \frac{1}{2j} [k+1] + \left(-\frac{1}{2j}\right) [k-1] \right) = \frac{1}{4j} \delta[k+1] + \frac{1}{4j} \delta[k-1]
$$
即 $c_{-2} = \frac{1}{4j}$，$c_2 = -\frac{1}{4j}$，其余 $c_k = 0$。
...
即
$$
c_2 = \frac{1}{4j}，c_{-2} = -\frac{1}{4j}， \text{其余} \, c_k = 0。
$$
