...
#show: template-post.with(
  title: "VSCode 写 C",
  description: "一句话：VSCode 确实是一个好东西",
  tags: ("VSCode", "C",),
  category: "环境搭建",
  date: datetime(year: 2021, month: 12, day: 29)
)

== 前言

#quote[
在用 VSCode 前，我写 C 一直用的是 VS2019 ，直到我看见了 GZTime 学长写的 #link("https://blog.gztime.cc/posts/2020/6b9b4626/")[Visual Studio Code Guide] 一文并照着配了后，我乐了，原来 VSCode 如此好用。首先作为一个编辑器，它比 VS2019 这个 IDE 小了太多（一个几十 MB ，一个 10 个 GB ），其次，它配上 gcc 可以避免 VS2019 由于 msvc 导致的 `scanf_s` 等一堆 `_s`（或许我的表述不是太严谨，但差不多是这个意思）。
本文高度概括了学长的文章，说是转载也不为过，读者根据自身水平可酌情移步至#link("https://blog.gztime.cc/posts/2020/6b9b4626/")[原文]。
]
...
#link("https://wwi.lanzouw.com/b00v71rpc")[下载链接]
...
+ 打开 VSCode 至你的文件夹（此处以 `C:\Coding` 为例）。
+ 打开左侧 `Extensions` 选项卡，搜索 C++ 并安装 `C/C++` 和 `C/C++ Intellisence` 两个扩展。
+ 在根目录新建文件夹 `Scripts` 以及 `Debug`
+ 在 `Scripts` 文件夹中新建文件 helloworld.cpp ，写下传统的 helloworld 程序：
...
+ 在 `.vscode` 文件夹中新建文件 `tasks.json` 并输入如下内容：
...
+ 类似的，配置 `c_cpp_properties.json` ，这个我不知道要不要（逃
...
#quote[
#link("https://blog.gztime.cc/posts/2020/6b9b4626/")[Visual Studio Code Guide] by GZTime
]
#link("https://blog.csdn.net/m0_55005568/article/details/119960552")
