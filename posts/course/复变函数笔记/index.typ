...
*复数的开方*
复数开方时，开几次方就有几个结果。
求 $w=root(n, z)$ 时，令 $w= rho e^("i" phi.alt )$ ，从而 $rho ^n e^("i"n phi.alt ) = z = re^("i" theta )$ ，得到
...
$ frac( diff ^2 phi.alt , diff x^2) + frac( diff ^2 phi.alt , diff y^2) = 0 $
...
$ f(z_0) = frac(1, 2 pi "i") integral.cont _Cfrac(f(z), z-z_0)"d"z $

*高阶求导公式* 设 $f(z)$ 在 $D$ 内解析，在 $D$ 的边界 $C$ 上连续， $C$ 为正向简单闭曲线，则 $f^((n))(z)$ 在 $D$ 内解析，且有
$ f^((n))(z_0)=frac(n!, 2 pi "i") integral.cont _Cfrac(f(z), (z-z_0)^(n+1))"d"z, quad forall z_0 in D, n=0,1,2, dots.c $
...
*收敛半径* 求法与高数类似，但多个根值法：
（1）（比值法） $lim "limits"_(n -> infinity )|"fraca"_(n+1)a_n|=L$
（2）（根值法） $lim "limits"_(n -> infinity )root(n, |a_n|)=L$
则收敛半径 $R=frac(1, L)$
...
从定义上判断的话就先展开成洛朗级数，即设 $z_0$ 为 $f(z)$ 的孤立奇点，将 $f(z)$ 在 $0 < |z-Z_0| < delta$ 内洛朗展开为 $f(z)= sum "limits"_(n=0)^(+ infinity ) a_n(z-z_0)^n$ 。

+ 可去奇点：展开式中不含负幂次项，直观上看就是 $lim "limits"_(z -> z_0)f(z)=C$ 。
+ $N$ 阶极点：含有限多的负幂次项，且最高负幂次为 $N$ ，直观上看就是 $lim "limits"_(z -> z_0)f(z)= infinity$。
+ 本性奇点：含无穷多的负幂次项，直观上看就是 $lim "limits"_(z -> z_0)f(z)$ 不存在且不为 $infinity$ 。
...
+ $integral _0^(2 pi )R( cos theta , sin theta )"d" theta = integral.cont _(|z|=1)R(frac(z^2+1, 2z),frac(z^2-1, 2"i"z))frac(1, "i"z)"d"z= integral.cont _(|z|=1)f(z)"d"z=2 pi "i" sum "limits"_k "Res"[f(z),z_k]$ ，其中 $z_k$ 是 $f(z)$ *在 $|z|=1$ 内*的孤立奇点。
...
*计算方法*： $integral ^(+ infinity )_(- infinity )R(x)"d"x=2 pi "i" sum "limits"_k"Res"[R(z),z_k]$ ， 其中 $z_k$ 为 $R(z)$在*上半平面*内的孤立奇点。
...
&=2 pi "i" sum "limits"_k"Res"[R(z),z_k] \\
...
*计算方法*： $integral ^(+ infinity )_(- infinity )R(x)e^("i"ax)"d"x=2 pi "i" sum "limits"_k"Res"[R(z)e^("i"az),z_k]$ ， 其中 $z_k$ 为 $R(z)$在*上半平面*内的孤立奇点。
...
*例* 求矩形脉冲函数 $f(t)="begincases"1, & |t| <= a\\0, & |t|> a"endcases"$ 的傅里叶变换及傅里叶积分表达式。
...
&=frac(2, omega ) dot.op "frace"^(-"i" omega a)-e^("i" omega a)-2"i" \\
&= frac(2, omega ) dot.op sin a omega 
"endaligned" $
再作傅里叶逆变换，即得傅里叶积分表达式
$ "beginaligned"
f(t)="mathscrF"^(-1)[F( omega )]&=frac(1, 2 pi ) integral _(- infinity )^(+ infinity )frac(2 sin a omega , omega )e^("i" omega t)"d" omega \\
...
&="begincases"1, & |t| < a \ 1/2, & |t| = a \ 0, & |t| > a"endcases"
"endaligned" $
上式中令 $t=0$ ，可得重要积分公式
...
**解** 函数 $F(s)$满足条件 $ lim "limits"_(s -> 0)F(s)=0$ ，且 $s=0$ 是 $F(s)$ 的二级极点， $s=-1$是 $F(s)$的一级极点，故 $
\begin{aligned}
f(t) = \mathscr{L}^{-1}[F(s)]
