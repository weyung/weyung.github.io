...
_Amdahl 定律_：$t_(new) = t_(related)/S + t_(unrelated)$，$S$ 为_加速比_。

时钟周期数 = 指令数 x 平均每条指令所需周期数（即 cycles = IC x CPI）
CPU 执行时间 = 指令数 x CPI x 时钟周期长（即 CPUtimes = IC x CPI x T）
或 _CPUtimes = IC x CPI / f_

_能耗_ = 负载电容 x 电压^2 = $CU^2$
_功耗_ = 1/2 x 负载电容 x 电压^2 x 开关频率 = $frac12 CU^2 f$

_七个伟大思想_：使用抽象简化设计、加速大概率事件、通过并行提高性能、 通过流水线提高性能、 通过预测提高性能、存储器层次、通过冗余提高可靠性。

== 指令

MIPS 一共有 32 个 _32 位_的寄存器
程序中的变量放在_保存寄存器_（store reg）中：`$s0~7` 共 8 个
运算的临时变量放在_临时寄存器_（temp reg）中：`$t0~9` 共 10 个
_零寄存器_（zero reg）：`$zero`，常用于赋值为 0

三类指令：
_运算指令_：`add`、`sub`、`mul`、`div`、`and`、`or`、`xor`、`nor`、`slt`、`sll`、`srl`、`sra`
_数据传送指令_：`lw`、`sw`、`lui`
_决策指令_：`beq`、`bne`、`j`、`jal`、`jr`
...
要减就_把立即数设为负数_，故 MIPS 无 `subi` 指令。
...
`a[0]` 到 `a[5]` 的距离是 5 个字，而_在 MIPS 中，一个字是 4 个字节_，所以 `a[5]` 的地址是 `a` 的基址加上 20。
把 `$t0` 的值传送到 `a[2]`：
...
MIPS 有 32 个寄存器，所以 5 位就可以表示全部的 32 个寄存器，所以 _rs、rt、rd 都是 5 位_。
_R 型指令的 opcode 都是 0_，由 6 位功能码指定操作，`$t0 ~$t7` 的编号是 8-15，`$s0 ~$s7` 的编号是 16-23，`add` 的 funct 是 32，`sub` 的 funct 是 34。
...
`lw` 和 `sw` 也是 I 型指令，但是 _rt 表示目的寄存器，rs 表示基址寄存器，和上面相反_，imm 表示偏移量，这两个指令的操作码分别是 35 和 43。
...
支持过程的三大寄存器：4 个_参数寄存器_ `$a0 ~$a3`，两个_返回值寄存器_ `$v0 ~$v1`，_返回地址寄存器_ `$ra`。

`jal` 即 jump and link，跳转到标签并_保存返回地址_到寄存器 `$ra`，由调用者主程序使用。
`jr` 即 jump register，跳转到寄存器中的地址，由被调用者过程使用，一般是 `jr $ra`。

两个栈指针寄存器 `$sp`（stack pointer） 和 `$fp` （frame pointer），分别表示栈指针和帧指针。在过程调用时，_如果要用到保存寄存器，就要先压入栈中，调用结束后再弹出_。
...
_堆由低向高增长，栈由高向低增长_，双向奔赴，实现了内存空间的高效利用。

=== 寻址

R 型指令都是寄存器寻址。
I 型指令第三个操作数（即第二个源操作数）是立即数的指令采用立即数寻址。`lw` 和 `sw` 采用基址偏移寻址。`beq` 和 `bne` 采用 PC(program counter) 相对寻址，_分支 32 位地址 = PC + 4 + 字地址偏移量_。
J 型指令都是伪直接寻址，因为 J 型指令是由 6 位操作码和 26 位目标地址组成的，寻址时_先左移 2 位形成 28 位字节地址，再和 PC 的高 4 位拼接成 32 位地址_。
...
`bne $t0,$s5, Exit` 中的立即数应该是 2，这里可以大概理解成隔了两条指令，而 `j Loop` 中的立即数稍微麻烦，首先因为 PC 的高 4 位是一样的，都是全 0，所以只需要考虑低 28 位，`Loop` 的地址是 10000，右移 2 位就是 100，所以 `j Loop` 的立即数是 100 （十进制的 4）的_补码_。
...
_原码_：用最高位表示正负号，其余 7 位表示绝对值
_反码_：将最高位为 0 的原码按位取反表示负数
_补码_：按位取反再加 1
...
比较简单的 32 位 ALU 是行波进位加法器，即_每一位都是一个全加器_，每个全加器有三个输入：两个加数和上一位的进位，两个输出：和位和进位。
但是每次进位都要过一次或门和与门，就产生了 64 个门延迟。

为了加快进位，进而加速加法运算，现在广泛采用_超前进位加法器_，通过将进位分为 4 位一组，抽象成每组的进行，实现加法器的并行执行。

要执行 n 位加法，求出 n 是 4 的几次方，并向上取整，再乘 2 加 1，就是超前进位加法器的门延迟。
用数学表达式表示就是 $2 times ceil( log _4 n ) + 1$，_n 为 16 时是 5，n 为 32 和 64 时都是 7_。

除法中，规定_余数和被除数同号_，比如 -7/2 = -3 余 -1，而不是商 -4 余 1。
...
IEEE 754 标准规定了浮点数的表示方法，_32 位单精度浮点数由 1 位符号位、8 位指数位和 23 位尾数位组成_。注意_指数要加 127 的偏差_，同时这里的_尾数是指小数点后的部分_，因为二进制的科学计数法里，只要不是 0.0，那么小数点前的部分一定是 1。
举个例子 -0.00101 = -1.01 x 2^-3，很明显，因为是负数，所以符号位是 1，指数位是 -3 + 127 = 124，尾数位是 01（后面补 0）。
_64 位双精度浮点数由 1 位符号位、11 位指数位和 52 位尾数位组成，指数偏差是 1023。_
...
_组合逻辑不含存储器_，给定输入时输出唯一确定，如 ALU。
_状态逻辑（又称时序逻辑）含存储器_，至少拥有两个输入：数据输入和_时钟输入_和一个输出。
总线表示数据信息多于一位的信号线，在数字电路图中加粗表示并标记位宽，由于 MIPS-32 采用 32 位字，所以当总线为 32 位时不必写出位宽。

=== 指令周期

一条 MIPS 指令的执行分为 5 个阶段，统称一个_指令周期_，分别是：

+ 取指令（IF，Instruction Fetch）：从内存中取指令
+ 指令译码（ID，Instruction Decode）：译码并取寄存器
+ 执行（EX，Execute）：执行指令
+ 访存（MEM，Memory）：访问内存
+ 写回（WB，Write Back）：写回结果
...
_理想条件为：1.每个流水级时间等长 2.流水线没有开销 3.指令数足够多。_
省略流水周期可能导致两条指令抢占同一流水级的硬件部件，引发_结构冒险_（structural hazard），解决的通行方法是添加硬件。
...
解决办法：
1）_数据旁路_：将数据从一个流水级传递到另一个流水级，而不是等到写回阶段再传递。
2）_先写后读_：对寄存器堆操作时，前半个周期写，后半个周期读，可以减少一次数据冒险。
3）_插入气泡_：在流水线中插入一个空操作，让后面的指令等待，直到数据准备好。

ALU 运算结果引出旁路传到另一个 ALU，叫做 ALU-ALU 旁路，从 MEM 级传到 ALU 叫做 MEM-ALU 旁路，两个旁路都有时，称为_全旁路_。
...
解决办法：
1）_缩短分支延迟_：将分支地址计算从 EX 级提前到 ID 级；
2）_动态分支预测_：通过查找指令地址观察上一次执行该指令时分支是否发生，如果上次执行时分支发生，就从上次分支发生的地方开始取新的指令；
3）_延迟分支调度_：编译器通过分析代码用不影响分支的一条指令填充到分支延迟时间槽。

=== 流水线性能

流水线在同一时间处理多条指令的不同阶段，实现指令级并行。
由时钟周期长度决定的_吞吐率是评价流水线性能的重要指标_。
在两个流水级间插入_流水线寄存器_，以左右两个流水级命名，分别叫 IF/ID、ID/EX、EX/MEM、MEM/WB。
流水线的时钟周期是由最慢的流水级决定的。
流水线的控制信号在译码时产生，逐级递减。
_前半个周期写寄存器堆，后半个周期读寄存器堆，可以减少一次数据冒险。_
...
对一条程序正在执行的指令来说
如果该指令位于_循环体_中，很可能不久后会被再次访问——_时间局部性_
如果该指令位于_循环体_或_顺序指令流_中，将要执行的指令往往地址相近——_空间局部性_
...
CPU 访问内存时，都会优先访问 cache，如果 cache 中有数据，就是_命中_，否则就是_缺失_。
命中/缺失占访存次数的比例叫做命中率（HR，Hit Rate）和缺失率（MR，Miss Rate），满足 HR + MR = 1。

=== 访存阻塞周期数

CPU 访问 cache 的时间称为命中时间，通常只有 1T
CPU 访问内存比访问 cache _多出来的时间_称为_缺失代价_（Miss Penalty）
注意访问内存比直接访问 cache 多出来的_额外开销_，才是缺失代价
...
对 $2^n$ 个块的 cache，cache 块号 = 内存二进制_块号_的后 n 位
注意内存的块号是地址的前 30 位，
...
cache 最前面还要加上一个_有效位_，用于标记 cache 中的块是否有效

==== cache 位数计算

处理器访存读写的数据通常为一个 32 位字，_一字一块根本不能利用空间局部性_。
...
对每个块，还要有一个_有效位_，所以一个块就有 1 + 18（地址标记） + 16*8 = 147 个 bit
总容量就是 147 x 1024 = 150528 bit = 18792 B = 18.375 KB
注意这里如果题目换成 16 KB 的 cache 也_默认是 16 KB 数据的 cache_，不是 16 KB 容量的 cache。

==== 缺失分类 3C 模型

根据产生原因，缺失分为以下三大类：

+ 首次访问 cache 中没有的块必然产生的缺失，称为_冷启动强制缺失_（Compulsory Miss）
+ 由于 cache 的容量不能容纳程序执行需要的所有块，部分块被替换后再次调入 cache，称为_容量缺失_（Capacity Miss）
+ 多个内存块竞争映射到同一个 cache 块，导致仍需使用的块被替换，称为_冲突碰撞缺失_（Conflict Miss）
...
+ 将 PC + 4 - 4（即当前指令的地址）写回 PC，并阻塞处理器
+ 访问内存，将内存块写入 cache
+ 再次访问 cache 并命中

当 CPU 把新数据写入内存块时，又要把数据写到 cache，保持内存和 cache 内容一致
方式一：CPU 和 cache 同时开始写内存，称为_写直达_（Write Through），但是开销很大，可改进为_缓冲_，CPU 较快写入缓冲后执行其他操作，缓冲再慢慢写入内存
方式二：CPU 只写入 cache，仅当这个 cache 块被替换时才写入内存，称为_写回_（Write Back）

=== 全相联映射

内存块可能映射到任何一个 cache 块，称为_全相联映射_（Fully Associative Mapping）
内存块可进入任何一个 cache 块，只要 cache 中有空位，就可以直接进入，不会产生冲突缺失
如果 cache 已满，则替换掉最长时间没有使用过的内存块，称为_最近最少使用_（Least Recently Used，LRU）替换策略
查找时需要比较每一块的标记位，开销过大，只适用于块数较少的 cache

=== 组相联映射

将直接映射和全相联映射折中，对 cache 块进行分组，一个内存块直接映射到一个组，但分配到组内的哪一块较为自由，也就是在一个组内全相联，这种映射方式称为_组相联映射_（Set Associative Mapping）
一组包含 n 块则称为 n 路组相联，其相联度为 n
直接映射就是 1 路组相联，全相联就是相联度 = 块数的组相联

cache 组号 = 内存块号 % cache 组数
对 $2^n$ 个组的 cache，cache 组号 = 内存二进制_块号_的后 n 位
...
_例_ 假设一个 cache 有 4096 个块，块大小为 4 个字，地址为 32 位，请计算两路组相联、全相联中 cache 的总位数。
...
_平均访存时间 = 命中时间 + 缺失率 x 缺失代价_（AMAT = HT + MR x MP）

_例_ 设处理器基本 CPI 为 1，时钟频率为 4 GHz。设访存指令占指令数的 20%，一级 cache 缺失率为 10%，速度可满足处理器全速运行；现增加一个二级 cache，访问时间为 5 ns，全局缺失率为 2.5%，访问主存的时间为 100 ns。二级 cache 局部缺失率和该处理器的 CPI 为多少？
...
使两个核的写操作一前一后的机制称为_写串行化_（Write Serialization）
...
虚拟内存的空间大于物理内存
磁盘和内存之间交换的块称为_页_（Page）
对 4 KB 即 $2^(12)$ B 的页，需要 12 位表示页内偏移
虚拟页号通过查询页表（Page Table）转换为物理页号，由于内存和磁盘中页大小相同，_页内偏移和物理页号拼接形成物理地址_

=== 页表和页表项 页表寄存器 缺页处理

每个进程都有一张页表，存放自己的内存空间
页表首地址存放在_页表寄存器_（Page Table Register，PTR）中
页表项记录每个虚拟地址对应的物理地址
和高速缓存类似，页表中需要一个有效位来识别访问的页是否在内存中
访问页不在内存时，产生一次_缺页_，此时需要读取磁盘用于扩充物理内存的部分，即_交换区_（Swap Space）

=== 替换策略 写策略

当一个进程的物理内存空间占满时，从磁盘交换区中调入内存的块需要替换掉内存中的某一块
使用_引用位_（Reference Bit）来近似实现 LRU 替换算法

由于磁盘访问时间长达几千万个时钟周期，所以需要尽量减少访问磁盘特别是写入磁盘的操作
因为内存中的页只能使用写回机制写入磁盘
还需要在页表中添加一个_脏位_（Dirty Bit），用于标记页是否被修改过，被修改过的才需要写回磁盘
...
奇偶校验码距为 2，能实现_检测一位错_（Single Error Detection，SED），但不能定位错误
...
----|---|---|---|---|---|---|---|---|---|----|----|----|---
分类位号 | _p1_ | _p2_ | d1 | _p4_ | d2 | d3 | d4 | _p8_ | d5 | d6 | d7 | d8 | -
...
第四章讨论指令级并行，这一章讨论的是_任务级并行_，主要通过_多核_和_多线程_实现。
单指令流多数据流（SIMD）计算机对_向量_数据进行操作，性能优于标量机。
Roofline 模型被广泛用于评价并行_浮点_计算机的性能。

汇编语言拥有_编译器输出语言_和_编程语言_两种角色。
使用汇编语言编程的情况：
...
#link("https://www.bilibili.com/video/BV1je4y1Q7BK")
