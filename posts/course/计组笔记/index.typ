...
用数学表达式表示就是 $2 times ceil( log _4 n ) + 1$，*n 为 16 时是 5，n 为 32 和 64 时都是 7*。
...
+ 首次访问 cache 中没有的块必然产生的缺失，称为*冷启动强制缺失*（Compulsory Miss）
+ 由于 cache 的容量不能容纳程序执行需要的所有块，部分块被替换后再次调入 cache，称为*容量缺失*（Capacity Miss）
+ 多个内存块竞争映射到同一个 cache 块，导致仍需使用的块被替换，称为*冲突碰撞缺失*（Conflict Miss）
