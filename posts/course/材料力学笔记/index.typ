...
+ 弹性阶段：应力 $sigma$ 与应变 $epsilon.alt$ 成正比，即有 $sigma = Eepsilon.alt$ ，$E$ 为*弹性模量*。直线最高点对应的应力 $sigma _"p"$ 称为*比例极限*，超过这个比例极限后，还有一个*弹性极限*，这两点间虽然不是直线，但松开后变形还是可以*完全消失*的，但两点非常接近，所以实际上不作严格区分。
...
$sigma _x,sigma _y,tau _(xy)$ 均为已知量，可此式是一个以 $sigma _alpha$ 和 $tau _alpha$ 为变量的圆方程，以横坐标表示 $sigma$ ，纵坐标表示 $tau$ ，则圆心横坐标为 $frac(1, 2)(sigma _x+sigma _y)$ ，纵坐标为零，半径为 $sqrt((frac(sigma _x-sigma _y, 2))^2 + tau _(xy)^2)$ 。这一圆周称为*应力圆*。
