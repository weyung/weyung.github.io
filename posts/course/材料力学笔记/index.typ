...
使材料满足三个要求：_强度_、_刚度_、_稳定性_。

=== 变形固体的基本假设

三个假设：_连续性_、_均匀性_、_各向同性_。
...
设与横截面成 $alpha$ 角的斜截面 $k-k$ 的面积为 $A_alpha$，横截面的面积为 $A$，则
$ A_alpha = frac(A, cosalpha ) $

把应力 $p_alpha$ 分解成垂直于斜截面的正应力 $sigma _alpha$ 和沿斜截面的剪应力 $tau _alpha$，则
$ sigma _alpha = p_alpha cosalpha = sigma cos ^2alpha \\
tau _alpha = p_alpha sinalpha = sigma cosalpha sinalpha = frac(sigma , 2)sin 2alpha $

当 $alpha = 0$ 时，$sigma _alpha$ 达到最大值，即
...
+ 弹性阶段：应力 $sigma$ 与应变 $epsilon.alt$ 成正比，即有 $sigma = Eepsilon.alt$ ，$E$ 为_弹性模量_。直线最高点对应的应力 $sigma _"p"$ 称为_比例极限_，超过这个比例极限后，还有一个_弹性极限_，这两点间虽然不是直线，但松开后变形还是可以_完全消失_的，但两点非常接近，所以实际上不作严格区分。
+ 屈服阶段：一段小锯齿，应变_明显增加_，应力_先下降再小波动_，先下降的那个最低点为屈服阶段或屈服强度，记作 $sigma _"s"$ 。
+ 强化阶段：恢复抵抗变形能力，最高点对应应力 $sigma _"b"$ 为_强度极限_。
+ 局部变形阶段：过强度极限后出现缩颈
...
对塑性材料，$[sigma ] = frac(sigma _"s", n_"s")$；对脆性材料，$[sigma ] = frac(sigma _"b", n_"b")$。其中 $n_"s"$ 和 $n_"b"$ 称为_安全系数_，有
...
可以看出，对长度相同、受力相等的杆件，$EA$ 越大变形 $Delta l$ 就越小，所以 $EA$ 越大的材料越强，称为杆件的抗拉（压）_刚度_。

试验表明，应力不超过比例极限时横向应变 $epsilon.alt '$ 与轴向应变 $epsilon.alt$ 之比是一个常数，即
$ mu = -frac(epsilon.alt ', epsilon.alt ) $
$mu$ 称为横向变形因数或_泊松比_。之所以有个负号，是因为一般材料都是伸长时横向缩小，压缩时横向增大。

=== 轴向拉伸或压缩时的应变能

杆件拉伸时，有 $W = frac12FDelta l$，忽略动能、热能等变化，杆件就只存到了应变能 $V_epsilon.alt = W = frac12FDelta l = frac(F^2l, 2EA)$，比能 $v_epsilon.alt = frac12sigmaepsilon.alt$。
...
$ Delta l_T = alpha _lDelta Tdot l $
式中 $alpha _l$ 为材料的_线胀系数_。
...
由
$ 2pi times frac(n, 60) times M_"e" = P times 1000 $
得出计算外力偶矩 $M_"e"$ 的公式为
$ M_"e"_("Ndot m") = 9549 frac{P_"kW"}{n_"r/min"} $
...
式中 $W_"t"=I_p/R$ 为_抗扭(twist)截面系数_。
圆截面的抗扭截面系数为
$ W_"t" = frac(pi D^3, 16) $
空心圆截面的抗扭截面系数为
...
距离为 $l$ 的两个横截面之间的_相对转角_为
$ phi.alt = frac(Tl, G I_"p") $
$phi.alt$ 的变化率 $phi.alt '$ 为_单位长度扭转角_，单位为 rad/m，即
...
- 引起_拉伸_变形的轴力 $F_"N"$ 为正
- 使轴线_曲率增加_的弯矩 $M$ 为正
- 以剪力 $F_"S"$ 对所考虑的一段曲杆内任一点取矩，若力矩为_顺时针_方向，则剪力 $F_"S"$ 为正
...
梁中间段上剪力为零，弯矩为常量的情况称为_纯弯曲_。

梁发生弯曲变形时长度不变的纤维层称为中性层，中性层与横截面的交线称为_中性轴_。

=== 横力弯曲时的正应力

工程实际中觉的弯曲问题多为_横力弯曲_，此时梁的横截面上不仅有_正应力_而且有_切应力_。
...
$W$ 称为_抗弯截面系数_，与截面的几何形状有关，单位为 $"m"^3$ 。
...
_圆形_截面梁的最大切应力
$ tau _"max" = frac43frac(F_"S", pi R^2) $
为平均切应力的 $frac43$ 倍。

=== 提高弯曲强度的措施

对抗拉和抗压强度相同的材料（如碳钢）宜采用中性轴对称的截面，对抗拉和抗压强度不相等的材料（如铸铁）宜采用中性轴_偏向于受拉一侧_的截面形状。
...
发生弯曲变形时，变形前为直线的梁轴线，变形后成为一条连续且光滑的曲线，称为_挠曲线_。
...
_弯曲变形很小_且材料服从_胡克定律_时，挠曲线的微分方程是线性的。

== 应力和应变分析 强度理论

=== 应力状态概述

切应力等于零的面称为_主平面_，主平面上的正应力称为主应力。

=== 二向和三向应力状态的实例

圆筒的壁厚 $delta$ 远小于它的内径 $D$ 时，称为_薄壁圆筒_。若封闭的薄壁圆筒所受内压为 $p$ ，则其横截面上应力
$ sigma ' = frac FA = frac(pdot fracpi D^2, 4)pi Ddelta = frac(pD, 4delta ) $
纵向截面上应力
$ sigma '' = frac(pD, 2delta ) $
...
$sigma _x$ 和 $tau _(xy)$ 是_法线与 $x$ 轴平行_的面上的正应力和切应力；$sigma _y$ 和 $tau _(yx)$ 是_法线与 $y$ 轴平行_的面上的正应力和切应力。
符号规定：正应力拉正压负，切应力对单元体内任意点的矩为_顺时针_转向时为正，反之为负，这里与平常不同。
取任意斜截面，其外法线 $n$ 与 $x$ 轴的夹角为 $alpha$ 。规定：由 $x$ 轴转到_外法线_ $n$ 为_逆时针_转向时，则 $alpha$ 为正。
...
$ sigma _alpha = frac(sigma _x+sigma _y, 2) + frac(sigma _x-sigma _y, 2)cos 2alpha - tau _(xy)sin 2alpha \\
tau _alpha = frac(sigma _x-sigma _y, 2)sin 2alpha + tau _(xy)cos 2alpha $

=== 二向应力状态分析————图解法

上面两式两边平方然后相加可消去 $alpha$ ，得
$ (sigma _alpha -frac(sigma _x+sigma _y, 2))^2 + tau _alpha ^2 = (frac(sigma _x-sigma _y, 2))^2 + tau _(xy)^2 $
$sigma _x,sigma _y,tau _(xy)$ 均为已知量，可此式是一个以 $sigma _alpha$ 和 $tau _alpha$ 为变量的圆方程，以横坐标表示 $sigma$ ，纵坐标表示 $tau$ ，则圆心横坐标为 $frac(1, 2)(sigma _x+sigma _y)$ ，纵坐标为零，半径为 $sqrt((frac(sigma _x-sigma _y, 2))^2 + tau _(xy)^2)$ 。这一圆周称为_应力圆_。
...
$ sigma _"max" = sigma _1, quadsigma _"min" = sigma _3, quadtau _"max" = frac(sigma _1-sigma _3, 2) $
$sigma _2$ 就是一般就是垂直于 $sigma _1$ 和 $sigma _3$ 的应力。

=== 广义胡克定律

$ epsilon.alt _x = frac1E[sigma _x-mu (sigma _y+sigma _z)] $

$ epsilon.alt _1 = frac1E[sigma _1-mu (sigma _2+sigma _3)] \\
epsilon.alt _2 = frac1E[sigma _2-mu (sigma _1+sigma _3)] \\
epsilon.alt _3 = frac1E[sigma _3-mu (sigma _1+sigma _2)] $
...
最大畸变能密度理论（第四强度理论）
$ sigma _("r"4) = sqrt(frac12[(sigma _1-sigma _2)^2+(sigma _2-sigma _3)^2+(sigma _3-sigma _1)^2]) $
...
按第三强度理论，有
$ sqrt(sigma ^2+4tau ^2) leqslant [sigma ] \\
frac1Wsqrt(M^2+T^2) leqslant [sigma ] $
按第四强度理论，有
$ sqrt(sigma ^2+3tau ^2) leqslant [sigma ] \\
frac1Wsqrt(M^2+0.75T^2) leqslant [sigma ] $

== 压杆稳定

=== 压杆稳定的概念

细长杆件受压时，设压力与轴线重合，压力小于某一极限值时，压杆一直保持_直线_形状的平衡，即便有微小的侧向干扰力使其暂时发生轻微弯曲，干扰力解除后，压杆也能恢复直线形状，这表明压杆直线形状的平衡是稳定的。但是如果压力大于某一极限值时，压杆的直线平衡变为不稳定，将转变为_曲线_形状的平衡。这时再用微小的侧向干扰力使其发生轻微弯曲，干扰力解除后，它将保持曲线形状的平衡，不能恢复到原有的直线形状。

上述压力的极限值称为临界压力或临界力，记为 $F_"cr"$ 。压杆丧失其直线形状的平衡而过渡为曲线平衡，称为丧失稳定性，简称_失稳_，也称为屈曲。
...
$ I = i^2A $
上式可以写成
$ sigma _"cr" = frac(pi ^2E, (fracmu l){i)^2A} $
引用记号
$ lambda = frac(mu l, i) $
...
分别定义为图形对 $z$ 轴和 $y$ 轴的_静矩_，也称为图形对 $z$ 轴和 $y$ 轴的_一次矩_。
这个坐标轴之所以只有 $y$ 和 $z$ 而没有 $x$ ，是因为我们一般分析的是横截面，$x$ 轴是杆的轴线方向。
可以看出，平面图形的静矩是对某一坐标轴而言的，也就是说，同一图形对不同的坐标轴的静矩通常是不同的。静矩的量纲是长度的三次方。

平面图形对 $y$ 轴和 $z$ 轴的静矩，分别等于_图形面积_ $A$ _乘形心的坐标_ $overline(z)$ 和 $overline(y)$ ，即
$ S_z = Acdotoverline(y), quad S_y = Acdotoverline(z) $

=== 惯性矩和惯性半径

在坐标 $(y,z)$ 处，取微面积 $"d"A$ ，遍及整个图形面积 $A$ 的积分
$ I_y = integral _A z^2"d"A, quad I_z = integral _A y^2"d"A $
分别定义为图形对 $y$ 轴和 $z$ 轴的_惯性矩_，也称为图形对 $y$ 轴和 $z$ 轴的_二次矩_。惯性矩的量纲是长度的四次方。
矩形的对形心轴的 $I_z$ 为 $frac(bh^3, 12)$ 。
力学计算中，有时把惯性矩写成图形面积 $A$ 与某一长度的平方的乘积，即
$ I_y = Adot i_y^2, quad I_z = Adot i_z^2 $
或者改写为
$ i_y = sqrt(frac(I_y, A)), quad i_z = sqrt(frac(I_z, A)) $
式中的 $i_y$ 和 $i_z$ 分别称为图形对 $y$ 轴和 $z$ 轴的_惯性半径_。惯性半径的量纲就是长度的量纲。

以 $rho$ 表示微面积 $"d"A$ 到坐标原点 $O$ 的距离，下列积分
$ I_"p" = integral _A rho ^2"d"A $
定义为图形对坐标原点 $O$ 的_极惯性矩_。又 $rho ^2 = y^2+z^2$ ，于是有
...
定义为图形对 $y,z$ 轴的_惯性积_。惯性积的量纲是长度的四次方。
