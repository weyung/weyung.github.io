...
第二天睡醒继续搞，思路是要拿到*前端的源代码*，才能搞懂他的逻辑。根据群友 \@xy3 提供的文章 #link("https://blog.cccyun.cn/m/?post=473") 试了一下小程序解包，发现 #link("https://github.com/biggerstar/wedecode")[wedecode] 已经可以直接一把梭了，不用文章前面提到的解密程序处理。
...
找了半天忘记用的是*企业微信*了，还在微信的目录里盯，于是在企微的文件夹（默认应该是 `C:\Users\用户名\Documents\WXWork`） 下打开终端，然后启动 wedecode，选择指定目录扫描，再输一个 `.` 表示当时目录，直接就定位到了签到的小程序，然后按一下 Enter 就直接顺利解包了。
...
这时就可以看出来之前看到的 `sKey` 其实是参数通过 *AES ECB mode 加密*后的东西，并且密钥 `sh12345678901234` 是*硬编码*的，用 CyberChef 尝试解密了一下果然没问题，那后面的工作就更方便了。
...
一开始没看出来不需要发送位置信息，一直在研究 `/sign/submitPosition` 路由的请求，但是把经纬度改了几次都不对，百度地图和谷歌地图的经纬度都试过了，然后忽然想到他用的是*腾讯位置服务*，于是在网站 #link("https://lbs.qq.com/getPoint/") 用坐标拾取器拿到经纬度交上去就通过了。
