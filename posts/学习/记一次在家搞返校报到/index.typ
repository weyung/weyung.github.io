...
#show: template-post.with(
  title: "记一次在家搞返校报到",
  description: "怎么 2 月没有 29 号和 30 号的。",
  tags: ("计算机", "学习",),
  category: "学习",
...
问了一下前两年毕业的老登，都说是用苹果来搞的，用的什么爱思助手 #strike[（有没有配套的爱慕助手）]
但是本人是安卓人，用的安卓手机和安卓电脑，于是就打算在 Windows 上试试先，实在不行再找苹果人借一台苹果手机和苹果电脑。

=== PC 端抓包

然后发现企微的小程序确实是能调用电脑的摄像头，_但是不能获取位置信息_，我也不知道怎么让他能获取到。
打开 Proxyman 尝试抓包，但是除了能看到获取列表信息和报到的信息外也没别的请求了。

=== 移动端抓包

上 B 站搜了一下手机抓包，看到有用 httpcanary（现已经更名为#link("https://reqable.com/")[reqable]）抓包的，装好证书开始抓包后，发现居然弹一个和电脑一样的报错框，说无法获取位置信息。
...
第二天睡醒继续搞，思路是要拿到_前端的源代码_，才能搞懂他的逻辑。根据群友 @xy3 提供的文章 #link("https://blog.cccyun.cn/m/?post=473") 试了一下小程序解包，发现 #link("https://github.com/biggerstar/wedecode")[wedecode] 已经可以直接一把梭了，不用文章前面提到的解密程序处理。
...
找了半天忘记用的是_企业微信_了，还在微信的目录里盯，于是在企微的文件夹（默认应该是 `C:\Users\用户名\Documents\WXWork`） 下打开终端，然后启动 wedecode，选择指定目录扫描，再输一个 `.` 表示当时目录，直接就定位到了签到的小程序，然后按一下 Enter 就直接顺利解包了。
...
这时就可以看出来之前看到的 `sKey` 其实是参数通过 _AES ECB mode 加密_后的东西，并且密钥 `sh12345678901234` 是_硬编码_的，用 CyberChef 尝试解密了一下果然没问题，那后面的工作就更方便了。
...
那么可以猜想，_位置信息本质其实还是在本地做校验_。根据这个思路，使用 requable 的重写功能改包，规则为发往 `/sign/submitPosition` 的请求返回值全部改为 0，然后就显示成功报到了。
...
一开始没看出来不需要发送位置信息，一直在研究 `/sign/submitPosition` 路由的请求，但是把经纬度改了几次都不对，百度地图和谷歌地图的经纬度都试过了，然后忽然想到他用的是_腾讯位置服务_，于是在网站 #link("https://lbs.qq.com/getPoint/") 用坐标拾取器拿到经纬度交上去就通过了。
