...
题目生成了一个元素在 $[-2^(15),2^(15)]$ 间的矩阵 $B_(75 times 150)$ ，和 $Z_N$ 上的矩阵 $A_(10 times 75)$ 。然后给了一个两矩阵相乘再模 $N$ 的结果 $C$ ，即 $C=ABmod N$ ，需要我们恢复出原来格$B$的最短向量。
...
0 & 0 & dots.c & 1 & 2^(200) dot.op c_(0,149) & 2^(200) dot.op c_(1,149) & dots.c & 2^(200) dot.op c_(9,149) \ "hline"
...
得到 $BC^T=Omod N$ ，结合上面 $BD^T=O$ ，推测。。。推测不出来了，然后结合队里大手子的分析如下

记题目中给出的两个矩阵为 $cal(A)$和$cal(B)$ ，有
 $cal(A)cal(B) = Cmod N$
若有 $B(cal(A)cal(B))^T = BC^T = Omod N$ ，则 $B$ 和 $C^T$ 互为左右零空间。
因此可以通过构造格 $L$ ，使得对格 $L$ 进行格基规约后可以得到一组基 $B$ 满足 $BC^T = Omod N$ ，即 $B$ 的基就是 $C^T$ 在模 $N$ 下的一个左零空间。
