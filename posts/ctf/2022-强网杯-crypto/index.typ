...
题目生成了一个元素在 $[-2^(15),2^(15)]$ 间的矩阵 $B_(75times 150)$ ，和 $Z_N$ 上的矩阵 $A_(10times 75)$ 。然后给了一个两矩阵相乘再模 $N$ 的结果 $C$ ，即 $C=ABpmod N$ ，需要我们恢复出原来格$B$的最短向量。

比赛时尝试过构造 $[ C space NI]^T$ ，跑LLL出来的结果很差， BKZ 的话一晚上啥也没出来。。
...
(
beginarraycccc|cccc
1 & 0 & dots.c & 0 & 2^(200)dot c_(0,0) & 2^(200)dot c_(1,0) & dots.c & 2^(200)dot c_(9,0) \\
0 & 1 & dots.c & 0 & 2^(200)dot c_(0,1) & 2^(200)dot c_(1,1) & dots.c & 0 \\
dots.v & dots.v & dots.down & dots.v & dots.v & dots.v & dots.down & 0 \\
0 & 0 & dots.c & 1 & 2^(200)dot c_(0,149) & 2^(200)dot c_(1,149) & dots.c & 2^(200)dot c_(9,149) \\ hline
0 & 0 & dots.c & 0 & 2^(200)dot N & 0 & dots.c & 0 \\
0 & 0 & dots.c & 0 & 0 & 2^(200)dot N & dots.c & 0 \\
dots.v & dots.v & dots.down & dots.v & dots.v & dots.v & dots.down & dots.v \\
0 & 0 & dots.c & 0 & 0 & 0 & dots.c & 2^(200)dot N
endarray
) $
可以看到 $A$ 左上角为一个 $150$ 维的单位阵，左下角为零阵，右上角为 $C$ 的转置数乘 $2^(200)$ ，右下角为一个 $10$ 维的单位阵数乘 $2^(200)dot N$ ，这里乘不乘 $2^(200)$ 得到的结果都是一样的，但神奇的是不乘的话 LLL 耗时会长一些。
$A$ 跑一遍 LLL 后，取结果的左上角 $75times 150$ 矩阵，记为 $B$ ，取 $B$ 的核记为 $D$ ，最后 $D$ 跑一遍 BKZ 后的最短向量即为所求。
#quote[
...
至于右上角为何是一个 $75times 10$ 的零阵，我也不知道，但 exp 既然这么断言，姑且就这么认为先，那么我们就推测
$ L=
[
beginmatrix
 B \\
...
得到 $BC^T=Opmod N$ ，结合上面 $BD^T=O$ ，推测。。。推测不出来了，然后结合队里大手子的分析如下

记题目中给出的两个矩阵为 $cal(A)$和$cal(B)$ ，有
 $cal(A)cal(B) = Cpmod N$
若有 $B(cal(A)cal(B))^T = BC^T = Opmod N$ ，则 $B$ 和 $C^T$ 互为左右零空间。
因此可以通过构造格 $L$ ，使得对格 $L$ 进行格基规约后可以得到一组基 $B$ 满足 $BC^T = Opmod N$ ，即 $B$ 的基就是 $C^T$ 在模 $N$ 下的一个左零空间。
