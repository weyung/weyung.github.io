...
不知不觉，离上一年 Hackergame 都一年了 #strike[（有点像废话）]，作为我CTF的启蒙赛， Hackergame 2021 拿分最多的其实还是靠 web ，如今我却出乎意料地成为了队里的密码手。

今年已经不像上年那么有空，前面的搜索题都没空做。总体来说，虽然做不出来，但好歹也能将大部分题目阅读一遍了。#strike[（有进步，但不多）]
...
第五问也没找出来，但是找到一个网站叫 #link("https://search.censys.io/data")[Censys.io] ，似乎可以搜索 ssh 的 sha256 指纹查 host ，以及有关一堆乱七八糟的查询，不知道以后能不能用得上。
...
Level2 打不通，但找到一个可以取消所有转义的函数 #strike[（但我忘了是什么）]。
...
第一回正式打逆向，首先题目是一个恶作剧程序，就是不给点 `狠心夺取` 按钮，于是拖进 `IDA` 看到有 `rand` 函数，不难联想到按钮的新位置是通过这个函数产生的，于是再查看 `rand` 函数的调用，发现只有两处 —— 分别是按钮的 X 坐标和 Y 坐标。

然后我的解法是把汇编上把位置 patch 成 `0`，这样按钮就只刷新在左上角了。

点击按钮后，弹出一个提示框——`获取flag失败！您不是本机的“超级管理员”！`，于是再次在 `IDA` 找到一个判断，若该变量不等于 `114514`，则弹出失败的提示框，这里在汇编把 `jz` patch 成 `jnz` 就行。最后成功获取 flag 。
...
那么这就很类似 RSA 了，首先 RSA 的解密原理是，对于模 $n$ 群，对其中的*任意元素* $x$，都有 $x^( phi.alt (n)) equiv 1mod n$，这里的 $phi.alt (n)$ 称为*群的阶*。\
所以
$
m^(e d) equiv m^(k phi.alt (n)+1) equiv (m^( phi.alt ))^k m equiv m mod n
$
（srds，这竟然是我在做这道题的时候推公式想明白的）
...
上面提到了*群的阶*，这里引入*元素的阶*的概念：比如模 $5$ 的群中，群的阶为 $phi.alt (5)=4$ ，也就是说 $1^4 equiv 2^4 equiv 3^4 equiv 4^4 mod 5$。\
但是我们发现，$1^1 mod 1$ 就已经是 $1$ 了，$4^2 mod 5$ 也一样不需要达到 $4$ 次方就到 $1$。\
那我们称*最小的* $k$ 使元素 $x$ 满足 $x^k equiv 1 mod n$ 为*元素 $x$ 的阶*，也就是说如上的例子中 $1$ 的阶为 $1$，$4$ 的阶为 $2$。

现在回到题目，我们发现难点在于要求的 $m$ 是比较大的，记 $g_1$ 和 $g_2$ 的阶分别为 $a$ 和 $b$，容易知道我们求离散对数得到的只是 $m_1 equiv m mod a$ 和 $m_2 equiv m mod b$，由中国剩余定理可知最后求出的 $m' equiv m mod "lcm"(a,b)$，也就是说，我们构造的 $g_1$ 和 $g_2$ 的阶的*最小公倍数需要足够大*才能得到 $m$。

然后不难搜得求置换群元素的阶最大的问题可以转化成求一组数的和不超过 $n$ ，如果使这组数的最小公倍数最大的问题。显然这组数应该是互质的，但*这并不意味着这组数要全为质数*！比如 $n=7$ 时，我们可以得到 $3 times 4$ 是最大的， $3$ 和 $4$ 互质，但 $4$ 不是质数。

如此我们可以手推几个，然后在数列网站 OEIS 上搜索得到数列 #link("https://oeis.org/A000793")[A000793]，关于这个数列的介绍为 `Landau's function g(n): largest order of permutation of n elements. Equivalently, largest LCM of partitions of n.`
...
查到三向量内积可表示成 $a^T B c$，其中 $B$ 的对角线为 $b$，但似乎没什么用。
用 z3 梭了几天出不来，放弃。
