...
#show: template-post.with(
  ...
  description: "很有意思的一道题，涉及私钥文件的结构",
  tags: ("CTF", "Crypto",),
  category: "CTF",
  date: datetime(year: 2022, month: 7, day: 16)
)
#line(length: 100%)
...
题目名为 _corrupted_key_ ，意为残损的私钥文件，既然是残损的，那么剩下的部分就是解题的关键了。
...
这里涉及一个_ASN.1_（Abstract Syntax Notation dot one，即抽象记法1）的问题，简单来说就是将数据编码成 3 个部分：_标志域、长度域、值域_
标志域中，约定 02 表示整数
长度域稍微复杂些，记录的是值域的长度，分为定长和不定长两种情况
定长时，若值域长度不超过 127 ，则用_短格式_表示，也就是直接用 16 进制表示，比如长度为 31 就是 0x1F ，即 0001 1111 ；若长度超过 127 ，则用_长格式_表示，首字节的首位置 1 表示长格式，后面7位则表示后面再跟多少个表示长度的字节，比如 1000 0001 表示后面 1 个字节表示长度，后面的长度也是直接用 16 进制表示。
...
#link("https://ctftime.org/task/2127")[0CTF 2016 Quals equation]

== 参考

#link("https://mp.weixin.qq.com/s/A9OmgHAmGLJPEL4cQBU8zQ")
#link("https://www.likecs.com/show-40060.html")
