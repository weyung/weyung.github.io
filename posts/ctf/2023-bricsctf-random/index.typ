...
可以获得_概率_线性关系，但是解线性系统就变成了 LPN（Learning Parity with Noise, 带噪声的学习奇偶校验）问题，据我所知，目前还没有足够快的算法。

#quote[
LPN 问题具体来说是这样的：给定一个二进制矩阵 $A$ 和一个噪声向量 $e$，求解未知的二进制向量 $s$ 使得 $As + e = bpmod2$。其中，$A$ 是一个随机生成的二进制矩阵，$s$ 是一个未知的二进制向量，$e$ 是一个含有少量随机错误的二进制向量，$b$ 是我们观测到的结果向量。噪声向量 $e$ 的存在使得这个问题变得非常困难。
]

这里可能有多个解；我找到的解是检查_二次关系_：
...
- 函数可以写成 $$f(x_1, \dots, x_{64}) = k_0 + \sum_{i} k_i x_i + \sum_{i \le j} k_{ij} x_i x_j,$$ 并且 `s1` 的每一位都可以表示为 $sum _i k_i s_i$，其中 s 是初始状态。
- 我们可以直接将 `s1` 的位的表达式代入 $f$，展开括号并获得对初始状态为真的二次方程。

我们仍然需要找到这些二次布尔函数。这可以通过生成许多随机的 64 位向量来完成，这些向量对应于给定的 `result >> 56` 的值，为它们中的每一个计算所有 $1 + 64 + frac64 dot 632 = 2081$ 个单项式，并找到矩阵的核。实现在 `gen_rels.cpp` 中，它平均每个输出字节找到 23 个方程。

这使我们能够在 256 个变量上创建一个超定的二次方程组，它的初始状态是它的（希望是唯一的）解。我用来解决它的算法是线性化：我们可以将每个单项式（如 $x_1x_2$）视为单独的变量。然后我们将有一个线性方程组，其中 $1 + 256 + frac256 dot 2552 = 32897$ 个变量，可以使用 M4RI 解决。实现在 `hax.cpp` 中，它期望 `gen_rels.cpp` 的输出在文件 `list_rels` 中，`output.txt` 的十六进制解码版本在文件 `out_bin` 中。
...
对每一个泄露的字节，找到所有 `krels` 中对应的 `big_rels`，然后对每一个 `big_rels`，新建一个类型为 `deg2_rel` 的 `res`，常数项（0 号位）和 `big_rels` 取等，1-64 号位是 $(b_1, b_2, dots.c , b_(64))dot (s_1,s_2,dots.c ,s_(64))^T$，左边的 $b$ 即为 `big_rels` 的 1-64 号位，右边是 s1 的 64*256 矩阵。后面没看懂。
...
#link("https://github.com/colgreen/Redzen/blob/main/Redzen/Random/Xoshiro256StarStarRandom.cs")
#link("https://github.com/C4T-BuT-S4D/bricsctf-2023-stage1/blob/master/tasks/crp/random/README.md")
