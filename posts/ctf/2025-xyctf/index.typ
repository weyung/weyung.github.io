...
随便试几下发现 `username` 字段可以注入，`password` 输什么都会被转义，且存在空格过滤，参考#link("https://www.freebuf.com/articles/web/426189.html")[这篇文章]使用 `tab` 作为空格绕过，payload 如下：
...
一点点把东西全注出来，然后用拿到的帐号密码还有 secret 直接登录，进入一个命令执行的页面，无回显，测试了一下存在空格过滤，于是用 `$IFS` 作为空格绕过，试了一下 `sleep${IFS}2`，发现响应明显变慢了，说明可以执行，执行 `ls$IFS/${IFS}>$IFSflag.txt`，看到 `flag.txt` 了，直接 `cp${IFS}/flag.txt${IFS}flag.txt`，再访问 `/flag.txt` 即可。
...
给了一个 `task.exe` 和 `flag.txt.enc`，用 IDA 一看都是 modules 之流，用 #link("https://github.com/WithSecureLabs/python-exe-unpacker")[python-exe-unpacker] 解包，得到的 `task` 文件就是 `task.sage`，代码如下：
...
我就说这个问题肯定有人想过的，搜到#link("https://crypto.stackexchange.com/questions/80834/attacks-on-lcg-with-self-xor-output-function")[这篇文章]
...
#link("https://www.ek1ng.com/SEKAICTF2022.html")
#link("https://ucasers.cn/python%E5%8F%8D%E5%BA%8F%E5%88%97%E5%8C%96%E6%BC%8F%E6%B4%9E%E4%B8%8E%E6%B2%99%E7%AE%B1%E9%80%83%E9%80%B8/")
#link("https://blog.csdn.net/xuxingzhuang/article/details/117108502")
#link("https://www.freebuf.com/articles/web/426189.html")
