...
+ `pass = ifstmt.getStmt(0)`：表示 `ifstmt` 的第一个语句是 `pass`。
+ `pass instanceof Pass`：表示 `pass` 是一个 `Pass` 语句。
...
#link("https://codeql.github.com/docs/codeql-language-guides/basic-query-for-python-code/")
