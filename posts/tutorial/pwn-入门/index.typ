...
#show: template-post.with(
  title: "pwn 入门",
  description: "浅记一下这些天学 pwn 的历程",
  tags: ("CTF", "二进制安全", "pwn",),
  category: "Tutorial",
  date: datetime(year: 2022, month: 7, day: 18)
)
#line(length: 100%)
...
_ELF_（Executable and Linkable Format）即 Linux 下的可执行文件格式，和 Windows 下的 _PE_ 格式类似，但 Linux 中的可执行文件一般不会加后缀名。
ELF 文件中有多个节（ Section ），主要有：

- _.text_ 节：即代码段，用来放代码
- _.rodata_ 节：ro 即 read only ，_只读_数据段，用来放静态数据，如果尝试修改会报错
    > `.rdata` 和 `.rodata` 的区别：两者都是只读数据段，但 `.rdata` 是 Windows 的常用说法，而 Linux 中则一般称 `.rodata`
- _.data_ 节：数据段，存放_可修改_的数据
- _.bss_ 节：中文不知道叫什么名字的段，也是放_可修改_的数据，但是_没有初始化_，所以不占ELF文件的空间，程序运行时会自动分配内存
- _.plt_ 节和 _.got_ 节：外部调用段（也不知道叫什么名字，这个是 AI 给我打的），调用动态链接库的函数的时候会用到
...
+ _NX_ ( No eXecute )(没错 X 就是大写，没打错)
    基本规则为_可写权限_与_可执行权限_互斥，即可被修改写入 shellcode 的内存都不可执行，被执行的代码数据不可修改，至于 shellcode 是啥，后面再提。
...
+ _IDA_
    ...
    分 32 位和 64 位两个版本，_这个打开不行就换另一个_，虽然我也不知道为啥不加个自动识别（）
    把程序拖进去，弹出一个奇怪的选项框，初学者直接 `enter`或者点 `OK` 就完事，然后进到 `IDA View-A` 标签页，这里初始时一般是流程图的形式，在此标签页按空格可以切换到普通模式，_记住不是在 `Pseudocode-A` 按_
...
+ _pwndbg_
    ...
    pwn 里面少不了本地调试，正常人都不能肉眼分析，那么就要用到 gdb ，but 裸的 gdb 太朴素了，不能满足人们日益增长的对优雅的追求，所以就有了_颜值极高_的 gdb 插件—— pwndbg
    安装方式：
    ...
    pwndbg 界面由上至下依次为
    - _legend_：图例，一般指示黄色为 Stack（栈），蓝色为 Heap（堆），红色为 Code（代码），紫色为 Data（数据），白色下划线为RWX（不知道啥），白色为 Rodata（只读数据）
    - _registers_：显示 CPU 的寄存器值
    - _disasm_：显示当前地址的反汇编代码
    - _stack_：显示栈的内容
    - _backtrace_：显示调用堆栈（我也不知道具体干嘛的）
    ...
    - `b` 设置断点，后面加地址或者函数名，比如 `b _0x400000` 或者 `b main` ，后者是在 main 函数入口处设置断点，或者 `b _main+111` 在 main 函数的第 111 条指令处设置断点
...
- 《CTF竞赛权威指南.Pwn篇》——杨超[著]
- #link("https://blog.csdn.net/sui_152/article/details/121650341")
- #link("https://stackoverflow.com/questions/65745514/what-is-the-difference-between-rodata-and-rdata")
- #link("https://blog.csdn.net/weixin_52553215/article/details/120690453")
- #link("https://blog.csdn.net/zino00/article/details/122716412")
- #link("https://blog.csdn.net/Demondai999/article/details/123875264")
