...
#show: template-post.with(
  title: "Docker 安全问题浅析",
  description: "学习一下常用的逃逸手法，以及如何设计一个安全的方案。",
  tags: ("Web", "渗透",),
  category: "渗透",
  date: datetime(year: 2025, month: 4, day: 3)
)
...
但是，这个 User Namespaces _不是默认开启的_，需要在 Docker 的配置文件中进行设置。
也就是说，_默认情况下，Docker 里的 root 跟宿主机的 root 就是一回事_，被面试官忽悠了，绷。
...
+ 命令行解析：Docker CLI 解析用户输入的命令行参数。
+ _API 调用：Docker CLI 通过 HTTP API 调用 Docker 守护进程。_
+ Dockerd 处理请求：Docker 守护进程接收请求并解析参数。
+ Containerd 和 runc：Docker 守护进程将请求传递给 containerd，containerd 负责容器的生命周期管理。runc 是一个低级别的容器运行时，负责创建和管理容器的命名空间、cgroups 等。
...
现在说说原理：Linux 有一个核心转储（core dump）机制（这中文听着怪怪的哈），当_进程崩溃时_，内核会将进程的内存映像保存到一个文件，用于调试。

而 `/proc/sys/kernel/core_pattern` 定义了核心转储文件的生成规则。其格式分为：

+ 静态路径：如 `/var/crash/core.%p`（`%p` 表示进程 PID）。
+ 管道命令：以 | 开头时，内核会将 core dump 内容通过管道传递给指定程序，格式为：
...
OverlayFS 则是 Linux 内核提供的一种 _UnionFS 的具体实现_。
OverlayFS 需要四个目录：

+ _lowerdir（下层目录）_：只读的基础层（如 Docker 镜像）。
+ _upperdir（上层目录）_：可写层，存放修改后的文件。
+ _merged（合并目录）_：最终用户看到的统一视图，合并了上下层内容。
+ _workdir（工作目录）_：系统内部用于处理文件操作（如临时存放复制的文件）。

_读取_文件时，若文件在 `upperdir` 中存在，则读取 `upperdir` 中的文件；否则读取 `lowerdir` 中的文件。
_修改_ `lowerdir` 中的文件时，会触发写时复制（Copy-on-Write）机制，将文件复制到 `upperdir` 中进行修改，原文件保持不变
_删除_文件时，会在 `upperdir` 中标记一个“删除白板”，隐藏下层文件。
最终效果：通过 `merged` 目录，用户可以看到一个合并后的完整的文件系统视图，原始基础镜像（lowerdir）始终不变。

注意到上面提到的_写时复制_，这是应用非常广泛的一种设计模式。
可以概括出如下优点：

- _高效_：无需复制整个基础层，只有修改时才复制单个文件。
- _节省空间_：多个容器可以共享同一基础层，避免重复存储。
- _快速启动_：创建新容器时，只需创建新的 `upperdir` 和 `workdir`，而不需要复制整个文件系统。
...
+ _宿主机的操作系统_本身就存在安全隐患
    我们知道，Docker 跟宿主机是_共享内核_的，所以如果宿主机的内核存在漏洞，那么 Docker 也会受到影响。比如著名的脏牛提权漏洞（CVE-2016-5195）。
+ _容器自身_的安全问题
    + 滥用 Docker API 攻击
    + Docker 逃逸攻击
    + 容器间通信的风险
...
容器的安全防护应该从容器的整个生命周期来考虑，包括一个容器镜像从_创建、传输、运行到停止_的全过程。
...
#link("https://docs.docker.com/engine/security/userns-remap/#about-remapping-and-subordinate-user-and-group-ids")[Isolate containers with a user Namespaces]
#link("https://zhuanlan.zhihu.com/p/679328995")[Docker 魔法解密：探索 UnionFS 与 OverlayFS]
#link("https://wiki.teamssix.com/")[T Wiki]
