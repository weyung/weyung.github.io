...
+ *API 调用：Docker CLI 通过 HTTP API 调用 Docker 守护进程。*
...
+ *lowerdir（下层目录）*：只读的基础层（如 Docker 镜像）。
+ *upperdir（上层目录）*：可写层，存放修改后的文件。
+ *merged（合并目录）*：最终用户看到的统一视图，合并了上下层内容。
+ *workdir（工作目录）*：系统内部用于处理文件操作（如临时存放复制的文件）。
