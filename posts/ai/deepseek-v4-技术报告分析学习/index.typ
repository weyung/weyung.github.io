...
+ 用 #link(<csa>)[CSA] 和 #link(<hca>)[HCA] 来增强长 context 下的效率
+ 用 #link(<mhc>)[mHC] 增强传统的残差连接
+ 用 #link(<muon-optimizer>)[Muon optimizer] 来做到更快的收敛和更稳定的训练
...
+ #text(fill: teal)[Q (Query / 查询)]：当前词正在*寻找*什么信息？
+ #text(fill: teal)[K (Key / 键)]：每个词拥有什么*特征*？
+ #text(fill: teal)[V (Value / 值)]：每个词包含的*具体内容*是什么？
