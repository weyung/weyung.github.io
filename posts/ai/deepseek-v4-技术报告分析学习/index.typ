...
+ *计算相似度。*$QK^T$ 就是让 $Q$ 矩阵（对单个 token 来说是向量，这里可以先视作一维矩阵）和 $K$ 矩阵相乘 #strike[（不会还有人不知道维度相同的矩阵相乘要转置的吧）]，两个向量的相似度越高，点积就越大，表示匹配程度越高。注意这里的 $Q$ 和 $K$ 在 Transformer 里并没有归一化，其欧几里得范数（即模长，也叫 L2 范数）*不是相等的*。

+ *归一化权重。*$d_k$ 为 $K$ 向量的维度，因为向量点积的结果是随维度增大而增大的，这样会导致一个问题：#link("#softmax-函数")[softmax 函数]作为一个用指数来归一化的函数，会把差异放得非常大，这样会导致 —— 其中某个值特别大的时候，其概率分布会趋近于 1，其他的概率分布都会趋近于 0，进而导致在反向传播计算时，梯度会趋近于 0，即陷入*梯度消失（Vanishing Gradient）*。所以在喂进 softmax 函数前，我们要先做一个*缩放（Scale）*，让数值落在 Softmax 梯度丰富的敏感区域。
