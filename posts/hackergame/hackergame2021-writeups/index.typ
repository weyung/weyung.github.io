...
#show: template-post.with(
  ...
  description: "第一次打CTF比赛",
  tags: ("CTF",),
  category: "Hackergame",
  date: datetime(year: 2021, month: 11, day: 5)
)

== Summary

作为新手，第一次参加 CTF ，题目比 MSClub 的招新 puzzle 难得多，但是也好玩得多，前两天有幸短暂地在总榜停留过（或者只是第一天？忘了），最终组内排名第四（因为校内的大佬不在）（ GZTime tql ，凌晨肝神冲到总榜第四orz，此处附上 #link("https://blog.gztime.cc/posts/2021/9f04efbd/")[wp 传送门]）。

比赛的题目出得都非常得有趣 #strike[（除了透明的文件）]，对新手来说体验确实不错，也确实是名副其实的 pythongame（零基础强行入门），做到灯那题的时候我面对线性非齐次方程组的无穷多解毫无头绪，这彻底燃起了我学线代的激情（嗯，有意思），最后我交的第一个错误的 flag 上了花絮也有小小的惊喜（没错我确实是有备而来 hiahiahia）。

总体体验良好，明年再来。下面是我做出的几题的 wp，内容较水，可酌情移步至#link("https://mcfx.us/posts/2021-10-30-hackergame-2021-writeup/")[神（mcfx）的 wp]。
...
// TODO: 下载图片 https://github.com/USTC-Hackergame/hackergame2021-writeups/raw/master/official/%E8%BF%9B%E5%88%B6%E5%8D%81%E5%85%AD%E2%80%94%E2%80%94%E5%8F%82%E4%B8%8A/src/hex_editor.png
// #image("hex_editor.png")
...
题目说了速度变了，显然是变快了，直接用经典音频隐写软件 Audacity 打开， `Ctrl` + `A` 全选，效果-改变速率，然后随手一拖把改变百分比拖到 `70` 就有非常标准清晰的单词，不看题目都能盲猜是首字母，后来查电报区分字符串中读音相近的字母的方法也确实对应，但是我还是不想自己听，打开翻译软件，嗯，语音识别不错，除了两个花括号我一开始没反应过来把也当成字母了 QAQ。
...
题目的废话文学着实是让我涨了见识，这简直简直了，我以前从未想过有如此废话的废话文学，等我看见的时候已经看见了......

第一问也是最难的一问，把百度必应谷歌翻烂了也没有，后来终于在一个专门存快照的国外网站上找到了那个已经消失的俱乐部的网页存档。

第三问直接在中科大 Linux 用户协会官网找到。

第五问也不难搜（虽然我没想到举报信可以发到这么奇怪的...呃不知怎么形容，目录？？去）。

第四问我看到论文直接望而生畏，搜索 data set 无果后果断爆破，合理猜测在 `100` 以下，但发现居然不行？？？万分怀疑的我试探性地把第二问改成 `3` （或许是我对近五年的理解有问题？），也不行，改成 `5` ，行了 #strike[（这协会真不要 face ）]，脚本如下：
...
这应该是让我学到最多东西的一题，看到题目的说明我第一反应就是通过 http 请求头伪造 IP 地址，搜到可以通过 X-Forwarded-For 实现。

但中途我试图通过审查元素修改表单数据实现，结果提示前后端地址不匹配（后来才发现我没读题）。

于是找了个伪造 IP 地址的插件，这次是后端改了 but 前端还是原来的。

两个结合，发现可以成功砍到，手改几次后发现速度来不及，被迫找到 Python 的实现，一开始没加时间限制，后来发现有令牌桶防着，就加了个停顿，脚本如下：
...
第一问不难，发现直接 `printf("Hello,world!")` 失败后改成用 `printf("%c",<每个字符的ASCII码>)`，但提交后错误。

仔细检查发现多了几个奇怪的不可显示字符，应该是直接从网页复制的问题，删掉后提交得到 flag，比赛结束后才知道还可以 putchar 呃呃
...
第一反应是 SQL 注入，但零基础的我忙活了半天才知道 `/graphql` 是端点。

查看 http 请求看到负载语句 `{query: "{ notes(userId: 2) { id↵contents }}"}` 猜测 admin 的 id 是 1 ，于是疯狂查询 1 的 contents ，结果回显都是我没有权限，后来搜到了 Graphql 的 IDE ，找到了存储邮箱的地方，查询得到 flag 。
...
数理杀我的一题， acm 的题中有过类似的，威尔逊定理求出 $p$，$q$ 的话不难求，很好，现在是经典 RSA 了，什么，你问我到底怎么求的 p？#strike[哦？我还等你讲给我听呢（装傻，阿巴阿巴）。]
...
继而由 $(x-1)!=y(y+1)(y+2) dots.c (x-1)$ ，不难推知

$
y! equiv -1(x-1)^(-1)(x-2)^(-1) dots.c (y+1)^(-1) mod x
$
