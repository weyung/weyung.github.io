...
作为新手，第一次参加 CTF ，题目比 MSClub 的招新 puzzle 难得多，但是也好玩得多，前两天有幸短暂地在总榜停留过（或者只是第一天？忘了），最终组内排名第四（因为校内的大佬不在）（ GZTime tql ，凌晨肝神冲到总榜第四orz，此处附上 #link("https://blog.gztime.cc/posts/2021/9f04efbd/")[wp 传送门]）。比赛的题目出得都非常得有趣 #strike[（除了透明的文件）]，对新手来说体验确实不错，也确实是名副其实的 pythongame（零基础强行入门），做到灯那题的时候我面对线性非齐次方程组的无穷多解毫无头绪，这彻底燃起了我学线代的激情（嗯，有意思），最后我交的第一个错误的 flag 上了花絮也有小小的惊喜（没错我确实是有备而来 hiahiahia）。总体体验良好，明年再来。下面是我做出的几题的 wp，内容较水，可酌情移步至#link("https://mcfx.us/posts/2021-10-30-hackergame-2021-writeup/")[神（mcfx）的 wp]。
...
#link("https://github.com/USTC-Hackergame/hackergame2021-writeups/raw/master/official/%E8%BF%9B%E5%88%B6%E5%8D%81%E5%85%AD%E2%80%94%E2%80%94%E5%8F%82%E4%B8%8A/src/hex_editor.png")[Hex]
...
题目的废话文学着实是让我涨了见识，这简直简直了，我以前从未想过有如此废话的废话文学，等我看见的时候已经看见了......第一问也是最难的一问，把百度必应谷歌翻烂了也没有，后来终于在一个专门存快照的国外网站上找到了那个已经消失的俱乐部的网页存档，第三问直接在中科大 Linux 用户协会官网找到，第五问也不难搜（虽然我没想到举报信可以发到这么奇怪的...呃不知怎么形容，目录？？去），第四问我看到论文直接望而生畏，搜索 data set 无果后果断爆破，合理猜测在 `100` 以下，但发现居然不行？？？万分怀疑的我试探性地把第二问改成 `3` （或许是我对近五年的理解有问题？），也不行，改成 `5` ，行了 #strike[（这协会真不要 face ）]，脚本如下：
...
首先科普一下模逆元的概念，比如 $xequiv 3^(-1)mod 5$ ，这个并非是 $x=frac(1, 3)$ 的意思，而是 $x$ 满足 $3xequiv 1 mod 5$ ，这时口算得出 $x=2$ ，因为 $6equiv 1 mod5$ 嘛。
