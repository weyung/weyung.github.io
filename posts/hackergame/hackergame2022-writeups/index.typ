...
Level2 一开始谷歌和百度识图都出不来，然后发现识图的结果都是夕阳，于是把图片_截去夕阳部分_再谷歌识图，就能得到拍摄角度几乎一样但是白天的日本千叶市美滨区的海洋球场，然后找机场就找头疼了，放弃。
...
那么这就很类似 RSA 了，首先 RSA 的解密原理是，对于模 $n$ 群，对其中的_任意元素_ $x$ ，都有 $x^(phi.alt (n))equiv 1mod n$ ，这里的$phi.alt (n)$称为_群的阶_。所以 $m^(ed)equiv m^(kphi.alt (n)+1)equiv (m^(phi.alt ))^kmequiv mmod n$ 。（srds，这竟然是我在做这道题的时候推公式想明白的）
...
上面提到了_群的阶_，这里引入_元素的阶_的概念：比如模 $5$ 的群中，群的阶为 $phi.alt (5)=4$ ，也就是说 $1^4equiv 2^4equiv 3^4equiv 4^4mod 5$ ，但是我们发现， $1^1 mod1$ 就已经是 $1$ 了， $4^2mod 5$ 也一样不需要达到 $4$ 次方就到 $1$ ，这里我们称_最小的_ $k$ 使元素 $x$ 满足 $x^kequiv 1mod n$ 为_元素 $x$ 的阶_，也就是说如上的例子中 $1$ 的阶为 $1$ ， $4$ 的阶为 $2$ 。
现在回到题目，我们发现难点在于要求的 $m$ 是比较大的，记 $g_1$ 和 $g_2$ 的阶分别为 $a$ 和 $b$ ，容易知道我们求离散对数得到的只是 $m_1equiv mmod a$ 和 $m_2equiv mmod b$ ，由中国剩余定理可知最后求出的 $m'equiv mmod "lcm"(a,b)$ ，也就是说，我们构造的 $g_1$ 和 $g_2$ 的阶的_最小公倍数需要足够大_才能得到 $m$ 。

然后不难搜得求置换群元素的阶最大的问题可以转化成求一组数的和不超过 $n$ ，如果使这组数的最小公倍数最大的问题。显然这组数应该是互质的，但_这并不意味着这组数要全为质数_！比如 $n=7$ 时，我们可以得到 $3times 4$ 是最大的， $3$ 和 $4$ 互质，但 $4$ 不是质数。如此我们可以手推几个，然后在数列网站 OEIS 上搜索得到数列#link("https://oeis.org/A000793")[A000793]，关于这个数列的介绍为 `Landau's function g(n): largest order of permutation of n elements. Equivalently, largest LCM of partitions of n.`
显然这正是我们要找的，于是复制下面给出的代码就可以生成 $g_1$ 了，而对于 $g_2$ ，我的做法是拿到 $g_2$ 中最大数后面的素数列填充进去，若不能继续填充且空间还有剩余，则使最后两个素数_尽可能大且尽可能相近_。
