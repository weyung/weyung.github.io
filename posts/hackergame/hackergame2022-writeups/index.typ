...
那么这就很类似 RSA 了，首先 RSA 的解密原理是，对于模 $n$ 群，对其中的*任意元素* $x$ ，都有 $x^( phi.alt (n)) equiv 1mod n$ ，这里的$phi.alt (n)$称为*群的阶*。所以 $m^(ed) equiv m^(k phi.alt (n)+1) equiv (m^( phi.alt ))^km equiv mmod n$ 。（srds，这竟然是我在做这道题的时候推公式想明白的）
...
上面提到了*群的阶*，这里引入*元素的阶*的概念：比如模 $5$ 的群中，群的阶为 $phi.alt (5)=4$ ，也就是说 $1^4 equiv 2^4 equiv 3^4 equiv 4^4mod 5$ ，但是我们发现， $1^1 mod1$ 就已经是 $1$ 了， $4^2mod 5$ 也一样不需要达到 $4$ 次方就到 $1$ ，这里我们称*最小的* $k$ 使元素 $x$ 满足 $x^k equiv 1mod n$ 为*元素 $x$ 的阶*，也就是说如上的例子中 $1$ 的阶为 $1$ ， $4$ 的阶为 $2$ 。
现在回到题目，我们发现难点在于要求的 $m$ 是比较大的，记 $g_1$ 和 $g_2$ 的阶分别为 $a$ 和 $b$ ，容易知道我们求离散对数得到的只是 $m_1 equiv mmod a$ 和 $m_2 equiv mmod b$ ，由中国剩余定理可知最后求出的 $m' equiv mmod "lcm"(a,b)$ ，也就是说，我们构造的 $g_1$ 和 $g_2$ 的阶的*最小公倍数需要足够大*才能得到 $m$ 。

然后不难搜得求置换群元素的阶最大的问题可以转化成求一组数的和不超过 $n$ ，如果使这组数的最小公倍数最大的问题。显然这组数应该是互质的，但*这并不意味着这组数要全为质数*！比如 $n=7$ 时，我们可以得到 $3 times 4$ 是最大的， $3$ 和 $4$ 互质，但 $4$ 不是质数。如此我们可以手推几个，然后在数列网站 OEIS 上搜索得到数列#link("https://oeis.org/A000793")[A000793]，关于这个数列的介绍为 `Landau's function g(n): largest order of permutation of n elements. Equivalently, largest LCM of partitions of n.`
