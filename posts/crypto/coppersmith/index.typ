...
首先是最简单的 $W$ ，论文明确给出 $W=||f(x_1X_n,"dots",x_nX_n)||\_ infinity$ ，而 $||f(x_1,"dots",x_n)||_( infinity )$ 的意思就是多项式的最大系数，所以 $W$ 就是多项式对各 $x$ 进行 $Xx$ 代入后的最大系数。如 $f=2x^2+3x+4$ ， $X=2$ ，则 $||f(x_1X_n,"dots",x_nX_n)||=8x^2+6x+4$ ，显然最大系数在 $x^2$ 那，即 $W=8$ 。
