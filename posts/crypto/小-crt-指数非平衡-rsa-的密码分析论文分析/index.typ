...
论文中写到 $e d=1 mod frac((p-1)(q-1), 2)$ （这个我没见过），然后 $gcd (p-1,frac(q-1, 2))=1$ ，再然后由CRT给出 $e d=1 mod p-1$ 且 $e d=1 mod frac(q-1, 2)$ 。
...
令 $q < N^ beta$ 且 $d_p <= N^ delta$ ，当满足 $3 beta + 2 delta <= 1 - log _N(4)$ 时可以在 $O( log ^2(N))$ 复杂度内分解 $N$ ，显然，这个方法只能在 $beta < frac(1, 3)$ 时有效。
*关键部分来了*，这里说到，文章中给出了一种构造任意维度的格的方法来将条件提升到 $3 beta - beta ^2 + 2 delta <= 1 - epsilon$ ，这个 $epsilon$ 是一个小错误的意思（我也不知道干嘛的）。因此只要 $beta < frac(3-sqrt(5), 2)=hat( phi )^2$ ，这个方法就是有效的，当 $hat( phi )$ 时是黄金比例的共轭（这一段我又不知道在干嘛了）。

=== 准备工作

记模 $N$ 环为 $bb(Z)_N$ ，整数模 $N$ 乘法群为 $bb(Z)^\*_N$
...
$ |k+1|=|frac(e d_p-2, p-1)| < frac(e d_p, p-1) < frac(q-1, 2)d_p < N^( beta + delta ) $
定义两个上界 $X=N^ delta$ 和 $Y=N^( beta + delta )$ ，则模二元多项式方程 $f_p$ 有一个小根 $(x_0),y_0$ 满足 $|x_0 | <= X$ 且 $|y_0 | <= Y$ ，这个模方程可以用 Howgrave_Grahm 定理转变为一个整数上的方程。
#quote[
*真相2 $("Howgrave-Graham")$*
令 $f(x,y)$ 为一个至多 $omega$ 个单项式的和的多项式，假定 $f(x_0,y_0)=0mod p^m$ ( $m$ 为正整数)， $|x_0 | <= X$ 且 $|y_0 | <= Y$ 。若 $||f(x X,y Y)|| < frac(p^m, sqrt( omega ))$ ，则 $f(x_0,y_0)=0$ 在整数上成立。
]

用格约化算法可以找到一个小欧里几得范数的多项式 $f(x X,y Y)$ ，第一个方法是构造一个二维的格，这时高斯约化算法可以找到一个最短向量。
我们选择 $m=1$ ，然后我们使用同样含根 $x_0 "modulo" p$ 的辅助多项式 $f_0(x)=N x$ ，因为 $N$ 是 $p$ 的倍数。
因此， $f_0$ 和 $f_p$ 的每个整数线性组合都有根 $(x_0,y_0) "modulo" p$ 。
此时构造一个由多项式 $f_0(x X)$ 和 $f_p(x X,y Y)$ 的系数向量张成的格 $L_p$ 。这些系数向量是如下 $(2 times 2)$ 格基矩阵 $B_p$ 的行向量
$ B_p=
mat(delim: "[",
 N X, 0;
 e X, -Y;
...
通过*引理3*，我们知道，对每个固定的 $epsilon > 0$ ，条件 $3 beta + 2 delta <= 1 - epsilon$ 有适当的模量 $N$ （这里我感觉不应该是这么翻译，后面再改）。
假定我们通过格约化找到了一个 $L_p$ 中的向量 $v$ ，其范数小于 $frac(p, sqrt(2))$ ，令 $v$ 为多项式 $f(x X,y Y)$ 的系数向量，应用*真相2*，我们知道 $f(x,y)$ 在整数上有一根 $(x_0,y_0)=(d_p,k+1)$ ，下一个定理将表明这个根可以很容易被找到。
#quote[
*引理4* 令 $v=(c_0,c_1) dot.op B_p$ 为 $L_p$ 的最短向量且满足 $||v||<frac(p, sqrt(2))$ 则 $|c_0 | = k$ 且 $|c_1 | = q d_p$
]
*证明* 略
...
#quote[
*定理5* 给定一个 RSA 公钥对 $(N,e)$ ，令 $q<N^ beta ,d_p <= N^ delta$ 且
$ 3 beta + 2 delta <= 1 - log_N(4) $
]
则 $N$ 可以在时间 $O(log^2(N))$ 内被分解。
*证明* 还是略（

在之前的分析中，我们假设 $q$ 不整除 $k$ ，特殊情况下若 $k=q r$ ( $r in bb(Z)$ )，我们得到了类似于以下更强结果之前的推理（百度翻译的）

#quote[
*定理6* 给定一个RSA公钥对$(N,e)$，令$q<N^ beta ,d_p <= N^ delta$且
$ k=q r "and" 3 beta + 2 delta <= 1 - log_N(4) $
]
则 $N$ 可以在时间 $O(log^2(N))$ 内被分解。
*证明* 又双叒叕是略（
...
定义 $x$ 移位多项式（这个也是百度翻译）
$ g_(m,i,j)(x,y)=N^(max(0,m-j)) x^i f^j_p (x,y) $
注意到，每个多项式 $g_(m,i,j)$ 的线性组合都有零点 $(x_0,y_0)=(d_p,k+1) "modulo" p^m$ 。
确定一个格维数 $n$ ，然后用 $g_(m,i,j)(x X,y Y)$ 的系数向量来构造一个 $n$ 维格 $L_p$ ，其中 $j=0...n-1$ ， $i=n-j-i$ 。参数 $m$ 是 $n$ 的函数且须进行优化。
比如选取 $n=4$ ， $m=2$ ，格 $L_p$ 由如下行向量张成
$ B_p=
mat(delim: "[",
 N^2 X^3, 0, 0, 0;
 e N X^3, -N X^2 Y, 0, 0;
 e^2 X^3, -2e X^2 Y, X Y^2, 0;
 e^3 X^3, -3e^2 X^2 Y, 3e X Y^2, -Y^3;
) $
...
$ 3 beta - beta ^2 + 2 delta <= 1 - epsilon $
]
...
#quote[
*引理8* 令 $X=frac(n+1, 2)N^ delta$ ， $Y=frac(n+1, 2)N^( beta + delta )$ ，多项式 $f_p(x,y)=e x-y$ 有一根 $(x_0,y_0) "modulo" p$ 满足 $|x_0 | <= N^ delta$ ， $|y_0 | <= N^( beta + delta )$ ，令 $v$ 为 $L_p(n)$ 中范数小于 $frac(p^m, sqrt(n))$ 的向量（ $v$ 为项式 $f(x X,y Y)$ 的系数向量），然后多项式 $p(x,y)=y_0 x-x_0 y in bb(Z)$ 一定整除 $f(x,y)$ ，我们就可以通过在 $bb(Z)[x,y]$ 上分解 $f$ 找到 $p$ 。
]
*证明* 略（
...
#quote[
*定理9* 给定一个 RSA 公钥对 $(N,e)$ ，令 $q < N^ beta ,d_p <= N^ delta$
]
且
$ 3 beta - beta ^2 + 2 delta <= 1 - epsilon $
