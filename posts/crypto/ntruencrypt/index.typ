...
#show: template-post.with(
  title: "NTRUEncrypt",
  description: "NTRUEncrypt 公钥加密系统",
  tags: ("数学", "格", "抽象代数", "Crypto",),
  category: "Crypto",
  date: datetime(year: 2022, month: 4, day: 11)
)
...
NTRUEncrypt 是一个公钥加密系统，它的安全性基于这样一个问题的困难性：在一个截断多项式环（这个翻译怪怪的）中_将一个给定的多项式分解成两个系数非常小的多项式的商_。
由于加密与解密都只涉及简单的多项式乘法，故相比于其他的加密系统， NTRUEncrypt 的_效率会更高_。
具体来说， NTRU 的操作基于截断多项式环中的对象 $\ R=mathbb {Z [X]/(X^(N)-1)}$ 中的_卷积乘法_，并且环上的所有多项式的系数和次数都为不大于 $N-1$ 的整数。
...
#image("https://gimg2.baidu.com/image_search/src=http%3A%2F%2Fauthorize.zhongbi.net%2Fd%2Ffile%2Ftu%2F2018%2F12%2F27%2F0wlyvynx1dv.jpg&refer=http%3A%2F%2Fauthorize.zhongbi.net&app=2002&size=f9999,10000&q=a80&n=0&g=0n&fmt=auto?sec=1652234696&t=c5802c1784650149a8a25e0440448985")

+ Bob 根据选定的 $N,p,q$ 生成最高次为 $N-1$ 的 $f$ 和 $g$ _两个多项式_，并且系数在 $-1,0,1$ 中选取（可以认为这俩是在模 $X^(N)-1$ 的剩余类中）。 $f$ 还要满足_模 $q$ 和 $p$ 的逆元存在_，如果不满足，那就重新生成。
+ 分别计算 $f$ 模 $p$ 和模 $q$ 的逆元，即 $f_(p)$ 和 $f_(q)$ ，_保留 $f$ , $f_(p)$ 及 $g$ 作为私钥，公钥 $h=pf_(q) dot g\ mod q$ 。_
...
$ f(x)*g(x) = c(x) mod 3 $
其中
$ c_0 = sum _(i+j=0 mod N) f_i g_(0-i) = f_0g_0 + f_1g_2 + f_2g_1 = -1 times 3 + 4 times 5 + 1 times (-1) = -3 + 20 + (-1) = 16 \\
c_1 = f_0g_1 + f_1g_0 + f_2g_2 = -1 times (-1) + 4 times 3 + 1 times 5 = 1 + 12 + 5 = 18 \\
c_2 = f_0g_2 + f_1g_1 + f_2g_0 = -1 times 5 + 4 times (-1) + 1 times 3 = -5 + (-4) + 3 = -6 $
故
$ c(x) = 16 + 18x - 6x^2 $
代码如下：
...
#link("https://en.wikipedia.org/wiki/NTRUEncrypt")
#link("https://github.com/joannawetesko/NTRU-cryptosystem/blob/master/NTRU.sage")
#link("https://blog.csdn.net/sinat_36742186/article/details/83689529")
