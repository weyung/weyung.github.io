...
可能一开始接触 ZKP 会跟我一样迷糊，我现在的理解，大概就是 _Schnorr 协议和 ZK 的关系_ = _RSA 和非对称加密的关系_
...
合理性和完备性不再赘述，但是关于零知识性，我们可以很明显地发现，$V$ 可以利用这个协议来解密任何密文——只要他想，这是一个非常直观的感觉，下面我们可以进一步在定义上探讨他为什么不符合_零知识性_：

首先我们要明确一个基础的概念——这个_知识_是什么？
与生活中的知识不同，在密码学里，这个知识指的是“计算能力的提升”，泄露“知识”，即是使 $V$ 能计算出他原本无法计算出来的东西。
很显然，如果存在一个 $V$ 未知的消息 $m^_$，$V$ 得到了对应的密文 $c^_$，上面这个 RSA 的证明协议就可以使 $V$ 计算出他原本无法计算出来的 $m^*$，那么就发生了知识的泄露，也就不满足零知识性。

下面我们再来研究怎么比较严格地证明零知识性，这里需要引入一个全新的概念——_模拟范式 (Simulation Paradigm)_

假设在另一个平行时空里，同样存在一个 $P$，为了区别于现实世界的 $P$，我们可以把他记为 $P'$。
这两个世界别无二致——除了 $P'$ 拥有一个叫_时光回溯_的超能力。
那么以上面的 Schnorr 协议为例，借助这个神奇的外挂，$P'$ 完全可以在不知道私钥 $x$ 的情况下，完美伪造一份_看似真实_的交互记录：

+ $P'$ 随便选一个 $t$ 发给 $V'$
+ $V'$ 随机选一个数字 $c in bb(Z)_q$，发给 $P$
+ $P'$ 发动技能，_把时间调回到第一步_，此时因为已经把 $c$ 骗到手了，所以他只需随便选一个 $s$，再计算 $t'equiv g^sy^(-c)pmod p$，在第一步把 $t'$ 发过去，然后在原本的这轮交互里把 $s$ 发过去
+ 基于上一步的计算，$V'$ 验证 $g^s equiv t'dot y^cpmod p$ 自然成立，选择相信 $P'$

而对于上述的 RSA 证明来说，即便再怎么进行时光回溯，$P'$ 也无法在没有私钥的情况下把 $V'$ 给的密文解出来，所以不具有_零知识性_
