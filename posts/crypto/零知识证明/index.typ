...
这是一个基于离散对数的交互式 ZKP，情景是 $P$ 需要向 $V$ 证明自己知道私钥 $x$，使得公钥 $y=g^x mod p$，其中 $g$ 是有限域循环群的生成元，阶为 $q$。

+ $P$ 随机选一个数 $r in bb(Z)_q$，计算 $t=g^r mod p$，然后把 $t$ 发给 $V$
+ $V$ 随机选一个数字 $c in bb(Z)_q$，发给 $P$
+ $P$ 计算 $s=r+c x mod q$，然后把 $s$ 发给 $V$
+ $V$ 验证 $g^s equiv t dot.op y^c mod p$ 是否成立，成立就选择相信，反之则反之

先来看*完备性*，即知道了一定能证明：

$ g^s equiv g^(r+c x+n q) equiv g^r (g^x)^c (g^q)^n equiv t dot.op y^c mod p $

再看*合理性*，即不知道基本不可能证明：

逆推也十分自然，即需要找一个数 $s$，满足

$ g^s equiv t dot.op y^c equiv g^r g^(c x) equiv g^(r+c x) mod p $

那可不就是要找一个 $x$ 使得 $s=r+c x mod q$，容易看出，一个 $x$ 就对应一个 $s$，蒙对的概率就是 $1/q$，实际应用中 $q$ 往往是极大的，就算只选取 $2^(128)$ 次方的量级也基本没可能猜中。
...
+ $V$ 随机生成一个密文 $c$ 给 $P$
+ $P$ 计算 $m equiv c^d mod N$，然后发给 $V$
+ $V$ 验证 $m^e equiv c mod N$ 是否成立
...
很显然，如果存在一个 $V$ 未知的消息 $m^ast$，$V$ 得到了对应的密文 $c^ast$，上面这个 RSA 的证明协议就可以使 $V$ 计算出他原本无法计算出来的 $m^ast$，那么就发生了知识的泄露，也就不满足零知识性。
...
+ $P'$ 随便选一个 $t$ 发给 $V'$
+ $V'$ 随机选一个数字 $c in bb(Z)_q$，发给 $P$
+ $P'$ 发动技能，*把时间调回到第一步*，此时因为已经把 $c$ 骗到手了，所以他只需随便选一个 $s$，再计算 $t' equiv g^s y^(-c) mod p$，在第一步把 $t'$ 发过去，然后在原本的这轮交互里把 $s$ 发过去
+ 基于上一步的计算，$V'$ 验证 $g^s equiv t' dot.op y^c mod p$ 自然成立，选择相信 $P'$
