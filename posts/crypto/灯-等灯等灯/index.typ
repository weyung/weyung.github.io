...
#show: template-post.with(
  title: "灯，等灯等灯",
  description: "线性同余方程组与格基约化问题",
  tags: ("Python", "数学", "格", "抽象代数", "CVP",),
  category: "Crypto",
  date: datetime(year: 2022, month: 3, day: 31)
)
...
#link("http://202.38.93.111:12768/login?token=1:MEQCIBY0ubN3BOXYsuRdXzqWIWNf8Jx0Y6giZCSp/Rg8zdNwAiBMZGWaiIGLD33KTfQ5TTLejp9PwGp8Gg4HsJbdo8sVig==")
如果还没关的话应该能打开（）
#quote[
...
+ _准备数据及函数_
    ...
    - `from sage.all import *`
首先要_安装 Sagemath_，因为 Sagemath 在 Windows 下运行也是要虚拟出一个 Unix 环境，故建议在 WSL 里 `sudo apt install sagemath` ，可能中途要 `apt-get upgrade` 。
    - `sys.argv[1]`
`argv[0]` 是脚本名称（它是否为完整路径名取决于操作系统）。则 `argv[1]` 为省去后缀名的文件名，如 Level0 时为脚本名为 `0.py` ， `argv[1]` 为 0 。
    - `ban = list(map(lambda x: [y == 'X'for y in x], levels[level_id][1].split()))`
    `lambda` 是 Python 的一个关键字，可以用来定义_匿名函数_。
...
+ _准备矩阵系数_
            ...
            for x in range(i - 2, i + 3):
                for y in range(j - 2, j + 3):
                    if (0 <= x < 12) and (0 <= y < 12):
                        t[id(x, y)] = levels[level_id]#link("i, j, x, y")[0]
            m.append(t + [0])
    ```

    - `levels[level_id]#link("i, j, x, y")[0]`
    这里是在元组里放函数，注意到上面的 `Levels=[(levev01_val, '''...'''),(...),(...)]` ，所以 `levels[level_id][0]` 是 `levev01_val` 函数。这样的用法如：

        ```Python
        def add(x,y):return x+y
        tup=(add,1,2)
        print(tup#link("tup[1],tup[2]")[0])
        # 3
        ```
    ...
    这个函数的参数中 `i,j` 为操作的点(即按下的点)坐标， `x,y` 为受影响的点坐标，_返回值为受影响坐标的增量_。
    ...
    上面已经定义函数 `def id(x, y): return x _ 12 + y` ，即 `id` 函数把给定的_坐标_转为方阵_一维展开后的位置_，则此处把每个操作点对整个方阵 144 个位置影响(增量)后面补一个 0 （下面会解释为什么加一个 0 ）放入 `t[]` 中，此时t可以看作一个_向量*。
    每个 `t[]` 补入 `m[]` 后，此时 $m$ 是一个 (144-16)x(144+1) 即 _128x145_ 的矩阵，且第 145 列全为 0 ( 16 为 X 即_不可操作点的数量_)，如下：
$ m=beginpmatrix
 3 & 2 & 1 & 0 & 0 & dots.c & 0 \\
 2 & 3 & 2 & 1 & 0 & dots.c & 0 \\
 1 & 2 & 3 & 2 & 1 & dots.c & 0 \\
...
$ m=beginpmatrix
 ...
 0 & 1 & 2 & 3 & 2 & dots.c & 0 & 0\\
 dots.v & dots.v & dots.v & dots.v & dots.v & dots.down & dots.v & dots.v \\
 0 & 0 & 0 & 0 & 0 & dots.c & 3 & 0\\
 256 & 0 & 0 & 0 & 0 & dots.c & 0 & 0\\
 0 & 256 & 0 & 0 & 0 & dots.c & 0 & 0\\
 0 & 0 & 256 & 0 & 0 & dots.c & 0 & 0\\
 0 & 0 & 0 & 256 & 0 & dots.c & 0 & 0\\
 0 & 0 & 0 & 0 & 256 & dots.c & 0 & 0\\
 dots.v & dots.v & dots.v & dots.v & dots.v & dots.down & dots.v & dots.v \\
 0 & 0 & 0 & 0 & 0 & dots.c & 256 & 0 \\
 -189 & -189 & -189 & -189 & -189 & dots.c & -189 & 256\\
 endpmatrix $

    此时矩阵 $m$ 的前 272 个行向量(即除开最后一个行向量)的_整系数线性组合_即为操作后可以得到的方阵化为一维后的向量， Level0 时只需令该向量与解向量相等，而 Level1 和 Level2 则需要算出离解向量_最近_的可由_这 272 个向量整数系线性表出的向量_作为_新的解向量_，因为我们_不保证在一些点不可操作的前提下仍能整数系线性表出解向量_(_重点，敲黑板_) 。
    ...
    _这就是为什么最后要补一列零向量的原因。_
    ________________****
    同时上面有一个细节，那就是 $m$ 的最后一个行向量为负的解向量，结合神的解释，我发现，若矩阵中最后一个行向量的最后一个维度为大常数，其他行向量的最后一个维度为 0 ，那么 LLL 后最后这个行向量只是与其他行向量的线性组合进行_一次相加(或相减)_，也就是说，这个方法将 CVP 化为 SVP 问题时，_求解 SVP 的过程中不会对这个特殊行向量进行任何数乘！_ 那么将这个向量直接乘上-1然后加上原来的解向量再抹去最后一维就是CVP的解了。
    同时我观察了 m2 (即 `m.LLL` 后的结果)的特征，发现 273 个行向量中的 128 个均为零向量，恰为 273-145 个。这里有一个有趣的问题，那就是_这 273 组基是否可以互相线性表出_。

    在线性代数中，一般的，我们在欧几里得空间即_实数域_里讨论向量的_实数系_组合问题，此时 145 维向量空间里的每个向量都可以由 145 个线性无关的向量线性表出。而格中是在向量的_整数系_组合下讨论问题，此时情况就有所不同了，比如以 $(1,1)$ 和 $(-1,1)$ 为基张成的格并不包括 $(1,0)$ 等向量，即不存在整数 $m,n$ 满足 $m_(1,1)+n_(-1,1)=(1,0)$ 。
...
+ _找到 CVP 的答案_
    ...
    由上面分析可知，此时 `m[-1]` 的前 144 维就是_新的解向量_。至此， Level1 和 Level2 就可以化为 Level0 的解法了。
+ _跑高斯消元求解_
    ...
    这里取 $m$ 的前 (144-16)=128 个行向量的前 144 维进行_转置_作为系数矩阵与 $m$ 的最后一行向量的前 144 维变为列向量，合成增广矩阵，即为一个标准的非齐次线性方程组，即 $s$ 为一个如下的 144x145 的矩阵：
$ s=beginpmatrix
...
- #link("https://mcfx.us/posts/2021-10-30-hackergame-2021-writeup/")
- #link("https://docs.python.org/3/library/sys.html")
- #link("https://zhuanlan.zhihu.com/p/67978661")
