...
#show: template-post.with(
  title: "记一次在局域网搭网站",
  description: "在学校里用自己电脑( Windows10 环境)当服务器搭了个网站，仅可在校园网下访问，图一乐。",
  tags: ("Python", "Nginx", "Windows", "SSL",),
  category: "小玩意",
  date: datetime(year: 2022, month: 3, day: 17)
)
...
Flask 是一个好用且简单的 web 框架，更多操作可以参考 #link("https://flask.palletsprojects.com/")[Flask官网]。
`pip install flask`
当然前提得先装好 python ，不提。
...
Nginx 下载地址 #link("http://nginx.org/en/download.html")
建议下载稳定版的，即 `Stable version` 。
下载后直接解压即可，_不要双击 Nginx.exe_。用终端打开 Nginx 的目录，然后使用命令 `start nginx` 运行。
...
#link("https://letsencrypt.osfipin.com/")
这个网站似乎不错

==== 一些问题

DNS 的验证逻辑非常奇怪，建议添加 TXT 记录后_等待 10 分钟_左右再提交验证申请(我就是栽了几次都验证失败，浪费好多时间)。同时我在 Windows 用 Certbot 申请的时候出现了写入权限的错误，也是莫名其妙的，既然日志文件都能写在目录下，证书却写不了。
