...
#show: template-post.with(
  title: "一个简单的本地判题脚本",
  description: "在做程序设计作业的时候，题目经常是要输入 10 个数字，但用不起 Matrix 的我只能手动输入，不但麻烦，而且也很难模拟随机过程，想过做一个 OJ （在线判题平台），但又苦于不会用 Docker ，思来想去，就用 Python 写了个本地判题脚本，虽然还不能防恶意代码，也无法实现内存检查，但基本的输入输出还是能够做到，供君一乐。",
  tags: ("Python",),
  category: "小玩意",
  date: datetime(year: 2021, month: 12, day: 28)
)
#line(length: 100%)
...
鉴于本人是追求实用（能跑就行），同理猜测读者也不关心这到底怎么实现，或者说不着急关心，于是直接附上 Windows 环境下的 Python 源码，开盒即用，只需要你有一点 Python 的基础，会装 subprocess 和 tqdm 库就行，然后在脚本所在的目录新建一个名为`标准代码`的文件夹，并在里面放入一个`std.c`作为测试输出的基准参照，同理新建一个`测试代码`的文件夹并放入`test.c`作为被测试的代码，_同时安装好gcc_，运行脚本，就能大功告成了。
