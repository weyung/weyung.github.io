...
#show: template-post.with(
  title: "变量指针指向的常量",
  description: "关于 `char *s=\"ABCDE\"` 为什么是有问题的",
  tags: ("C",),
  category: "C语言",
...
当看到答案选 B 的时候，我人傻了，#strike[因为我上网查的答案是C]，纳闷着调试，结果如下：
#quote[
...
#quote[
题干说进行_字符串赋值操作_，而 C 选项是给_指针赋值_
]

#strike[这个解释离大谱]。

后来在 GZTime 及 Hanmur 学长的#strike[无语]点拨下，我注意到了 C 选项的一个问题。
...
_注意：_ 常量指针的声明格式为 `数据类型 const _ 指针变量` 或者  `const 数据类型 _指针变量。`
...
运行结果是：前两句可以正常执行，而执行第 4 句时则会弹出 _Segmentation fault_。
对程序进行分析可以知道，`char _s="ABCDE";`一句会把字符串存储到_只读*的 `rdata` 段，所以无法改变其中的内容。
也就是说，`char _s="ABCDE"`一句中，_s 指向的字符串是常量*。

== 总结

那么回到开始的问题，`char *s="ABCDE";` 为什么是错的
答案是，_它用一个变量指针指向了一个常量_！
正确的写法应为：
`const char *s="ABCDE";`
