...
function readSiteConfigInputs(siteConfigPath) {
  ...
  if (!raw) {
    return { websiteUrl: null, author: null, postUrlStyle: "slug" };
  }
  ...
  try {
    ...
    const postUrlStyle = String(parsed.postUrlStyle || "slug").trim();
    return { websiteUrl, author, postUrlStyle };
  } catch {
    return { websiteUrl: null, author: null, postUrlStyle: "slug" };
  }
}

// 检查文章中的代码块语言是否被 Shiki 支持
function validateCodeBlockLanguages(content, postTitle) {
  ...
  const supportedLangs = new Set([
    ...
    "berry", "bibtex", "bicep", "bird2", "blade", "bsl", "c", "c3", "cadence", "cairo", 
    "clarity", "clojure", "cmake", "cobol", "codeowners", "codeql", "coffee", "common-lisp", 
    "coq", "cpp", "c++", "crystal", "csharp", "cs", "css", "csv", "cue", "cypher", "d", "dart", "dax", 
    "desktop", "diff", "docker", "dockerfile", "dotenv", "dream-maker", "edge", "elixir", "elm", 
    "emacs-lisp", "erb", "erlang", "fennel", "fish", "fluent", "fortran-fixed-form", 
    "fortran-free-form", "fsharp", "gdresource", "gdscript", "gdshader", "genie", "gherkin", 
    "git-commit", "git-rebase", "gleam", "glimmer-js", "glimmer-ts", "glsl", "gn", "gnuplot", 
    "go", "golang", "graphql", "groovy", "hack", "haml", "handlebars", "haskell", "haxe", "hcl", 
    "hjson", "hlsl", "html", "html-derivative", "http", "hurl", "hxml", "hy", "imba", "ini", 
    "java", "javascript", "js", "jinja", "jison", "json", "json5", "jsonc", "jsonl", "jsonnet", 
    "jssm", "jsx", "julia", "just", "kdl", "kotlin", "kusto", "latex", "lean", "less", 
    "liquid", "llvm", "log", "logo", "lua", "luau", "make", "markdown", "marko", "matlab", 
    "md", "mdc", "mdx", "mermaid", "mipsasm", "mojo", "moonbit", "move", "narrat", "nextflow", 
    "nextflow-groovy", "nginx", "nim", "nix", "nushell", "objective-c", "objective-cpp", 
    "ocaml", "odin", "openscad", "pascal", "perl", "php", "pkl", "plsql", "po", "polar", 
    "postcss", "powerquery", "powershell", "prisma", "prolog", "proto", "pug", "puppet", 
    "purescript", "python", "py", "qml", "qmldir", "qss", "r", "raku", "razor", "reg", "rel", 
    "riscv", "rst", "ruby", "rb", "rust", "rs", "sas", "sass", "scala", "scheme", "scss", "shaderlab", 
    "shellscript", "shell", "sh", "bash", "shellsession", "smalltalk", "solidity", "sparql", "splunk", "sql", 
    "ssh-config", "stata", "stylus", "svelte", "swift", "system-verilog", "tasl", "tcl", 
    "tex", "toml", "tsx", "turtle", "twig", "typescript", "ts", "v", "vb", "verilog", "vhdl", 
    "viml", "vim", "vue", "vue-html", "vue-vine", "vyper", "wasm", "wenyan", "wgsl", "wikitext", 
    "wit", "wolfram", "xml", "xsl", "yaml", "yml", "zenscript", "zig",
  ...
  while ((match = codeBlockRegex.exec(content)) !== null) {
    ...
    if (!supportedLangs.has(lang)) {
      console.warn(`\n  [WARN] Post "${postTitle}" uses potentially unsupported code language: "${lang}"`);
    }
  }
}

// 并发提取每篇文章 metadata，汇总成 posts 列表。
async function collectPostMetadata(postTypFiles, jobs, paths, siteInputs = {}) {
  const tasks = postTypFiles.map((typFile, index) => {
    return {
      typFile,
      cacheHtmlPath: join(paths.metadataCacheDir, `meta-${index}.html`),
    };
  });
  ...
  const results = await runPool(
    ...
    async (task, index) => {
      try {
        ...
        let slug = pagePath;
        if (siteInputs.postUrlStyle === "short-hash") {
          slug = createHash("sha256").update(pagePath).digest("hex").substring(0, 8);
        }
        ...
        let parsed;
        try {
          const content = safeRead(task.typFile);
          validateCodeBlockLanguages(content, pagePath);
          parsed = parseJsonFromMetadataHtml(ROOT_DIR, safeRead(task.cacheHtmlPath), task.typFile);
        } catch (e) {
        ...
        return {
          ...
          post: {
            slug: slug,
            _pagePath: pagePath, // Used for matching assets mapping
        ...
        return { ok: false, message: `[${index}] ${normalizePosixPath(relative(POSTS_DIR, task.typFile))}: ${e.message}` };
      }
    },
    progressBar,
  );
  ...
  for (const item of results) {
    if (item.ok) {
      ...
      metadataErrors.push(item.message);
    }
  }

  if (metadataErrors.length > 0) {
    console.error("");
    for (const msg of metadataErrors) {
  ...
  if (posts.length === 0 && results.length > 0) {
    throw new Error("All posts failed metadata extraction");
  }
...
function makePostCompileTasks(postTypFiles, postsJsonPath, slugsJsonPath, dependencies, siteInputs = {}) {
  ...
  return postTypFiles.map((typFile) => {
    ...
    const outputRel = pagePathToOutputRel(`posts/${slug}`);
...
async function compileAndStageTypstTasks(tasks, stagingSiteDir, jobs, statusMap) {
  ...
  for (const item of results) {
    if (item.ok) {
      ...
      failedCount++;
      if (item.message) {
        compileErrors.push(item.message);
      }
    }
  }
...
function stageAssetGroup(sourceRoot, sourceFiles, stagingSiteDir, targetPrefix, statusMap, label, postPathToSlugMap = null) {
  const progressBar = createProgressBar(label, sourceFiles.length);

  for (const sourcePath of sourceFiles) {
    let sourceRel = normalizePosixPath(relative(sourceRoot, sourcePath));

    if (postPathToSlugMap) {
      // Find the longest matching pagePath prefix
      const parts = sourceRel.split('/');
      let matchedPagePath = "";
      let matchedSlug = "";
      for (let i = parts.length - 1; i >= 1; i--) {
        const potentialPagePath = parts.slice(0, i).join('/');
        if (postPathToSlugMap.has(potentialPagePath)) {
          matchedPagePath = potentialPagePath;
          matchedSlug = postPathToSlugMap.get(potentialPagePath);
          break;
        }
      }
      if (matchedPagePath) {
        sourceRel = sourceRel.replace(matchedPagePath, matchedSlug);
      }
    }
...
export async function buildSite({ outDirName, jobs, configPath, cacheRoot }) {
  ...
  const posts = await collectPostMetadata(postSource.typFiles, jobs, paths, siteInputs);
  // Remove internal properties before writing JSON
  const postsForJson = posts.map(p => {
    const copy = { ...p };
    delete copy._pagePath;
    return copy;
  });
  writeJson(paths.postsMetadataJsonPath, postsForJson);
  ...
  const compileFailed = await compileAndStageTypstTasks(allCompileTasks, paths.stagingSiteDir, jobs, statusMap);

  // 5) 合并静态资源（站点资源 / 文章资源 / 页面资源）。
  const postPathToSlugMap = new Map(posts.map(p => [p._pagePath, p.slug]));

  const siteAssets = walkFiles(ASSETS_DIR);
  console.log("Staging site assets...");
  stageAssetGroup(ASSETS_DIR, siteAssets, paths.stagingSiteDir, "assets", statusMap, "assets:site ");
  console.log("Staging post assets...");
  stageAssetGroup(POSTS_DIR, postSource.assetFiles, paths.stagingSiteDir, "posts", statusMap, "assets:post ", postPathToSlugMap);
  console.log("Staging page assets...");
  stageAssetGroup(PAGES_DIR, pageSource.assetFiles, paths.stagingSiteDir, "", statusMap, "assets:page ");

  // 6) 生成 RSS（输出根目录 rss.xml）。
  console.log("Generating RSS...");
  stageRss(postsForJson, outputSiteDir, paths.stagingSiteDir, paths.siteConfigPath, statusMap);

  // 7) 计算删除项并完成输出目录切换。
  console.log("Syncing output directory...");
  const syncResult = incrementalSync(paths.stagingSiteDir, outputSiteDir);

  const metadataFailed = postSource.typFiles.length - posts.length;
  const totalFailed = metadataFailed + compileFailed;

  if (totalFailed > 0) {
    console.log(`\nBuild completed with ${totalFailed} error(s)`);
  } else {
    console.log("\nBuild succeeded!");
  }
  console.log(`Out: ${outputSiteDir}`);
  console.log(`@ Compiled   : ${allCompileTasks.length - compileFailed}/${allCompileTasks.length}`);
  if (metadataFailed > 0) {
    console.log(`! Meta fail  : ${metadataFailed}`);
  }
  if (compileFailed > 0) {
    console.log(`! Build fail : ${compileFailed}`);
  }
